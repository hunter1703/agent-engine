package com.agentengine.engine.plugin;
...
/**
 * Engine base agent — carries {@link BaseAgentConfig} and delegates runtime to
 * subclasses.
 */
public abstract class Agent extends BaseAgent {
  ...
  protected Agent(Builder<?, ?> builder) {
    this(builder.name(), builder.description(), builder.subAgents(), builder.agentConfig(), builder.beforeAgentCallback(),
        builder.afterAgentCallback());
  }

  protected Agent(String name, String description, List<? extends BaseAgent> subAgents, BaseAgentConfig agentConfig,

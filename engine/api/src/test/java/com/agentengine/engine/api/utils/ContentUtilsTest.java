...
class ContentUtilsTest {
  ...
  void shouldExtractLatestUserTextFromContents() {
    final List<Content> contents = List.of(Content.builder().role("user").parts(List.of(Part.fromText("first"))).build(),
  ...
  void shouldFindLatestUserContentIndex() {
    final List<Content> contents = List.of(Content.builder().role("model").parts(List.of(Part.fromText("m"))).build(),
  ...
  void shouldExtractLatestUserTextFromRequest() {
    final LlmRequest request = LlmRequest.builder()
        .contents(List.of(Content.builder().role("model").parts(List.of(Part.fromText("m"))).build(),
  ...
  void shouldTreatBlankTextOnlyContentAsEmpty() {
    final Content content = Content.builder().role("user").parts(List.of(Part.fromText("   "))).build();
  ...
  void shouldTreatFunctionCallPartAsNonEmpty() {
    final Part part = Part.builder().functionCall(FunctionCall.builder().name("search").args(Map.of("q", "weather")).build()).build();

    assertThat(ContentUtils.isEmptyPart(part)).isFalse();
    assertThat(ContentUtils.isEmptyPart(Content.builder().role("model").parts(List.of(part)).build())).isFalse();
  }
}

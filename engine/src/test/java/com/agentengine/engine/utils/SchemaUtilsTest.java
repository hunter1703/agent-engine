...
class SchemaUtilsTest {
    ...
    final Schema schema =
        Schema.builder()
            ...
            .properties(
                Map.of(
                    "name",
                    Schema.builder().type(Type.Known.STRING).description("User name").build(),
                    "age",
  ...
  void addsNullableToAnyOfSchemas() {
    final Schema schema =
        Schema.builder()
            .anyOf(
                Schema.builder().type(Type.Known.STRING), Schema.builder().type(Type.Known.INTEGER))
            .nullable(true)
            .build();
    ...
    assertThat(anyOf.stream().map(entry -> ((JsonObject) entry).getString("type")).toList())

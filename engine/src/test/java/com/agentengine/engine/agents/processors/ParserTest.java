...
class ParserTest {
  ...
  void shouldKeepTransferFunctionStyleCallsAsText() {
    ...
    final Content content = Content.builder().role("model")
        .parts(List.of(Part.fromText("transfer_to_agent(\"story_phase_1_brief\")\n" + "transfer_to_agent(\"story_phase_2_outline\")")))
        .build();
    ...
    assertThat(parts.getFirst().text().orElse("")).contains("transfer_to_agent(\"story_phase_1_brief\")")
  ...
  void shouldKeepTransferCamelCaseFunctionStyleCallsAsText() {
    final Parser parser = new Parser("", true);
    final Content content = Content.builder().role("model").parts(List.of(Part.fromText("transferToAgent(\"story_phase_3_draft\")")))
        .build();
  ...
  void shouldNotStripFunctionStyleTransferCallsFromTextPart() {
    final Parser parser = new Parser("", true);
    final Content content = Content.builder().role("model")
        .parts(List.of(Part.fromText("Plan created.\ntransfer_to_agent(\"story_phase_1_brief\")\nProceeding."))).build();
  ...
  void shouldNotParseNonTransferFunctionStyleCallsFromText() {
    final Parser parser = new Parser("", true);
    final Content content = Content.builder().role("model").parts(List.of(Part.fromText("run_cmd(\"echo hi\")"))).build();

    final Content parsed = parse(parser, content);
    final List<Part> parts = parsed.parts().orElse(List.of());

    final List<Part> functionParts = parts.stream().filter(part -> part.functionCall().isPresent()).toList();
  ...
  void shouldDropEmptyPartsFromPreProcessedRequest() {
    ...
    final LlmRequest request = LlmRequest.builder()
        .contents(List.of(Content.builder().role("user").parts(List.of(Part.fromText("   "), Part.fromText("keep"))).build())).build();
  ...
  void shouldStripToolPartsFromPreProcessedRequestWhenToolCallingDisabled() {
    ...
    final Part functionCallPart = Part.builder()
        .functionCall(FunctionCall.builder().name("search").args(Map.of("query", "weather")).build()).build();
    final Part functionResponsePart = Part.builder()
        .functionResponse(FunctionResponse.builder().name("search").id("call-1").response(Map.of("result", "sunny")).build()).build();
    final LlmRequest request = LlmRequest.builder()
        .contents(
            List.of(Content.builder().role("user").parts(List.of(Part.fromText("keep"), functionCallPart, functionResponsePart)).build()))
        .build();
  ...
  private static Content parse(final Parser parser, final Content content) {
    return parser.postProcess(LlmResponse.builder().content(content).build()).content().orElseThrow();
  }
}

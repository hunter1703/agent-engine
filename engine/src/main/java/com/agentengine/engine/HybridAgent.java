...
public final class HybridAgent implements Agent {
    ...
    public HybridAgent(final LLMModel reasoningModel, final LLMModel routerModel, final ToolExecutor toolExecutor, final SessionStore sessionStore, final int invocationLimit, final String agentId) {
    ...
    private Message runReasonerWithRepair(final String sessionId, final String runId, final int maxRetries) {
        final List<Message> prompt = CollectionUtils.nullSafeMutableList(reasoningModel.getContextManager().buildPrompt(sessionId));
        Message response;
        try {
  ...
  private boolean containsPlanToolCalls(final List<ToolCall> toolCalls) {
    ...
    private List<PlanItem> generateTaskList(final String sessionId, final String runId, final Message message,
                                            final AgentListener listener, final Session session,
    ...
    private List<Map<String, Object>> planArgs(final Session session) {
        ...
        return session.getPendingPlan().stream()
                .map(item -> {

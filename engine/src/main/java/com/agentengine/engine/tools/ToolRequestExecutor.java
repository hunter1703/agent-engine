    ...
    private List<ToolCall> runToolAssistant(final String sessionId, final List<ToolRequest> toolRequests,
        ...
        do {
            ...
            try {
                ...
                throw new ModelInvocationException("tool-assistant-model", "Failed to generate tool assistant response",
                        e);
            }
            ...
            if (CollectionUtils.isNotEmpty(remainingRequests)) {
                ...
                sessionStore.appendMessage(getToolSessionId(sessionId), Message.user(TemplateUtils
                        .renderTemplateForName("hybrid/repair/empty_tool_call.txt",
                                Map.of("toolRequests", missingRequests))));
                repairAttempts++;
                if (repairAttempts > 3) {

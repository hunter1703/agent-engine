 ...
 * request continuation). Responsibilities:
 *
 * <ul>
 * <li>{@code turnComplete=true} on every non-partial response — each
 * LlmResponse is the end of a model generation turn regardless of whether it
 * carries a final answer or tool calls.
 * <li>Consumes the continuation flag on every non-partial response to prevent
 * cross-turn leakage.
 * <li>On final-answer responses: if continuation was NOT requested, sets {@code
 *       finishReason=STOP} (preserving any existing value). If continuation WAS
 * requested, omits {@code finishReason} — {@code BaseFlow} treats absence of
...
public final class TurnCompletionResponseProcessor implements ResponseProcessor {
  ...
  public TurnCompletionResponseProcessor(Integer turnsRemaining) {
    this.turnsRemaining = turnsRemaining == null ? Integer.MAX_VALUE : turnsRemaining;
  }
  ...
  public Single<ResponseProcessingResult> processResponse(final InvocationContext context, final LlmResponse response) {
    ...
    final boolean continuationRequested = RunUtils.getState(context).consumeContinuation();

 ...
 *
 * <p>Canonical Order: 1. Thoughts (thought == true) 2. Regular Text (thought == false &&
 * text.isPresent()) 3. Tool Calls (functionCall.isPresent()) 4. Tool Responses
 * (functionResponse.isPresent()) 5. Everything else
 *
...
public final class PartOrderingResponseProcessor implements ResponseProcessor {
  ...
  public Single<ResponseProcessingResult> processResponse(
    ...
    return response
        .content()
        .<Single<ResponseProcessingResult>>map(
            content -> {
              final List<Part> parts = content.parts().orElse(List.of());
              if (parts.size() <= 1) {
                return Single.just(
                    ResponseProcessingResult.create(response, List.of(), Optional.empty()));
              }

              final List<Part> orderedParts = new ArrayList<>(parts);
              orderedParts.sort(PART_COMPARATOR);

              final LlmResponse orderedResponse =
                  response.toBuilder()
                      .content(content.toBuilder().parts(orderedParts).build())
                      .build();

              return Single.just(
                  ResponseProcessingResult.create(orderedResponse, List.of(), Optional.empty()));
            })
        .orElseGet(
            () ->
                Single.just(

...
public class ConfigValidationService {
  ...
  private final List<ConfigRuleValidator<?>> ruleValidators;

  @Inject
  public ConfigValidationService(
      final Validator validator, final @Any Instance<ConfigRuleValidator<?>> allRuleValidators) {
    this.validator = validator;
    this.ruleValidators =
        allRuleValidators.stream().sorted(Comparator.comparingInt(ConfigRuleValidator::order)).toList();
  }
  ...
  private <T> void applyRuleValidators(final T value, final ValidationCollector errors) {
    for (final ConfigRuleValidator<?> candidate : ruleValidators) {
      if (candidate == null || !candidate.targetType().isInstance(value)) {
        continue;
      }
      final ConfigRuleValidator<T> validator = (ConfigRuleValidator<T>) candidate;

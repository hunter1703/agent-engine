...
class AgentRestAPITest {
  ...
  void invokeReturnsSessionAndResponse() {
    ...
    assertThat(response.getFinalAnswer()).isEqualTo("ok");
    assertThat(response.getThoughts()).isEqualTo("t");
  }
  ...
  void invokeHandlesNullEngineResponse() {
    ...
    assertThat(response.getSessionId()).isEqualTo("session");
    assertThat(response.getFinalAnswer()).isNull();
    assertThat(response.getThoughts()).isNull();
  }
  ...
  void buildPromptReturnsMessages() {
    ...
    assertThat(response.getMessages().getFirst().getRole()).isEqualTo("system");
  }
}

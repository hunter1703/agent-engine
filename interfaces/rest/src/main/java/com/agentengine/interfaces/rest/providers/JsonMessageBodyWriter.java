...
public class JsonMessageBodyWriter implements MessageBodyWriter<Object> {
    ...
    /**
     * A copy of the shared base mapper with an AG-UI {@link Event} mixin registered.
     * The mixin injects the {@code "type"} field into every serialized {@link Event}
     * — Jackson does not include interface methods that are not record components, so
     * without this the {@code type()} discriminator is silently omitted from the wire JSON.
     */
    private static final ObjectMapper MAPPER = JsonUtils.copyMapper()
            .registerModule(new SimpleModule("AGUIEventModule")
    ...
    /** Mixin that annotates {@link Event#type()} so Jackson serializes it as the {@code "type"} property. */
    ...
    private static final class RoleSerializer extends StdSerializer<Role> {
        ...
        @Override
        public void serialize(final Role value, final JsonGenerator gen, final SerializerProvider provider)

...
public class AgentRestAPI {
  ...
  @APIResponse(responseCode = "200", description = "SSE event stream", content = @Content(mediaType = SERVER_SENT_EVENTS, schema = @Schema(implementation = BaseResponsesEventData.class)))
  @APIResponse(responseCode = "400", description = "Invalid request parameters")
  @APIResponse(responseCode = "500", description = "Internal server error")
  ...
  private AgentRequest convertResponsesApiRequestToAgentRequest(final ResponsesApiRequest request) {
    ...
    agentRequest
        .setSessionId(StringUtils.isBlank(request.getSessionId()) ? randomUUID().toString() : request.getSessionId());

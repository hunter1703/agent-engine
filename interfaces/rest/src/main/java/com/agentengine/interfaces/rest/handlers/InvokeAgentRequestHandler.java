...
public class InvokeAgentRequestHandler extends AbstractAgentRequestHandler<AgentResponse> {
  ...
  public AgentResponse handle(final AgentRequest request) {
    ...
    LOG.info(
        "Agent invocation handler started - agent_id={} session_id={} operation=agent.invoke.start",
        request.getAgentId(),
        request.getSessionId());
    LOG.debug(
        "Agent invocation handler details - agent_id={} session_id={} message_length={}",
        request.getAgentId(),
        request.getSessionId(),
    ...
    try {
      final AgentRunResult result =
          AgentRunResult.fromEvents(agentExecutionService.run(request).toList().blockingGet());
      LOG.info(
          "Agent invocation handler completed - agent_id={} session_id={} operation=agent.invoke.complete outcome=success",
          request.getAgentId(),
          request.getSessionId());
      LOG.debug(
          "Agent invocation handler response - agent_id={} session_id={} response_length={} thoughts_length={}",
          request.getAgentId(),
          request.getSessionId(),

...
public class ResponseAPI {
  ...
  public ModelList models() {
    Query query = new Query().withPage(new Page(0, 1000)).withSort(new Sort(BaseEntity.FIELD_ID, Sort.Order.ASC));
    PaginatedResult<BaseAgentConfig> agents = agentService.findAgents(query);
    return new ModelList(
        agents.getItems().stream().map(agent -> new Model(agent.getId(), "model", agent.getCreatedTime() / 1000, "agent-engine")).toList());
  }
  ...
  public Flowable<ChatCompletionChunk> chatCompletions(ChatCompletionsRequest request, @Context HttpHeaders headers) {
    ...
    return agentExecutionService.run(agentId, sessionId, prompt).map(new CompletionsEventMapper(completionId, agentId)::mapEvent)
        .switchMap(e -> e);
  }
  ...
  public Flowable<ResponseOutputEvent> createResponse(ResponsesRequest request, @Context HttpHeaders headers) {
    ...
    return agentExecutionService.run(modelId, sessionId, prompt).map(new ResponsesEventMapper(responseId, modelId, createdAt)::mapEvent)
        .switchMap(e -> e);
  }

...
public class ModelResourceTest {
  ...
  public void testCreateModel() {
    String payload = "{\"id\":\"" + MODEL_ID
        + "\",\"name\":\"QA Test Model\",\"type\":\"gemini\",\"model\":\"gemini-2.0-flash\"}";
    given().contentType(ContentType.JSON).body(payload).when().post("/v1/model").then().statusCode(200);
  }

  @Test
  @Order(2)
  public void testGetModel() {
    given().when().get("/v1/model/" + MODEL_ID).then().statusCode(200).body("id", equalTo(MODEL_ID));
  }
  ...
  public void testUpdateModel() {
    ...
    given().contentType(ContentType.JSON).body(payload).when().put("/v1/model/" + MODEL_ID).then().statusCode(200);
  }
  ...
  public void testGetBlankModel() {
    given().pathParam("id", " ").when().get("/v1/model/{id}").then().statusCode(400); // Bad Request because of
                                                                                      // invalid/blank ID
  }
  ...
  public void testCreateModelMissingFields() {
    ...
    given().contentType(ContentType.JSON).body(payload).when().post("/v1/model")
        ...
        .then().statusCode(org.hamcrest.Matchers.anyOf(equalTo(400), equalTo(500)));
  }

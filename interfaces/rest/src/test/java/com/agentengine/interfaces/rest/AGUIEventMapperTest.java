...
class AGUIEventMapperTest {
  ...
  void toolResultEndsStepBeforeAnswer() {
    ...
    final List<BaseEvent> events = Flowable.just(toolCallEvent, toolResultEvent, answerEvent).concatMap(mapper::map)
  ...
  void userTextDoesNotFinishStep() {
    ...
    final List<BaseEvent> events = Flowable.just(userEvent, modelEvent).concatMap(mapper::map)
        .concatWith(Flowable.defer(mapper::onComplete)).toList().blockingGet();

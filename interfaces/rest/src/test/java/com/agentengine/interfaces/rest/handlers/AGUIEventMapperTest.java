...
class AGUIEventMapperTest {
  ...
  void shouldPreserveDistinctRunIdsAcrossMultipleInvocationsInOneSessionHistoryReplay() {
    ...
    final Event second = Event.builder().invocationId("run-2").turnComplete(true).finishReason(new FinishReason(FinishReason.Known.STOP))
        .content(Content.builder().role("model").parts(List.of(Part.fromText("second"))).build()).build();

    final List<BaseEvent> mapped = mapper.map(first).concatWith(mapper.map(second)).toList().blockingGet();
    final List<String> startedRunIds = mapped.stream().filter(RunStartedEvent.class::isInstance).map(RunStartedEvent.class::cast)
        .map(RunStartedEvent::getRunId).toList();
    final List<String> finishedRunIds = mapped.stream().filter(RunFinishedEvent.class::isInstance).map(RunFinishedEvent.class::cast)
        .map(RunFinishedEvent::getRunId).toList();
  ...
  void shouldReuseSourceEventTimestampWhenReplayingStoredEvents() {
    ...
    final List<BaseEvent> mapped = new AGUIEventMapper("session-1", "agent-1").map(event).toList().blockingGet();
  ...
  void shouldPreserveDeterministicStepAndMessageIdsAcrossReplayReads() {
    ...
    final List<String> firstReplayIds = new AGUIEventMapper("session-1", "agent-1").map(event).toList().blockingGet().stream()
  ...
  void shouldFallbackToSequentialStepAndMessageIdsWhenSourceEventIdIsMissing() {
    ...
    final List<BaseEvent> first = new AGUIEventMapper("session-1", "agent-1").map(event).toList().blockingGet();
    final List<BaseEvent> second = new AGUIEventMapper("session-1", "agent-1").map(event).toList().blockingGet();
  ...
  void shouldKeepMessageIdsDisjointBetweenRunsAndStableAcrossReads() {
    ...
    assertThat(secondRead).withFailMessage(() -> "firstRead=" + firstRead + " secondRead=" + secondRead).isEqualTo(firstRead);
  }
  ...
  private Map<String, List<String>> mapMessageIdsByRun() {
    final Event first = Event.builder().id("event-1").invocationId("run-1").turnComplete(true)
        .finishReason(new FinishReason(FinishReason.Known.STOP))
        .content(Content.builder().role("model").parts(List.of(Part.fromText("first"))).build()).build();
    final Event second = Event.builder().id("event-2").invocationId("run-2").turnComplete(true)
        .finishReason(new FinishReason(FinishReason.Known.STOP))
        .content(Content.builder().role("model").parts(List.of(Part.fromText("second"))).build()).build();
  ...
  private Map<String, List<String>> mapMessageIdsByRunWithMissingIds() {
    final Event first = Event.builder().id(null).invocationId("run-1").turnComplete(true)
        .finishReason(new FinishReason(FinishReason.Known.STOP))
        .content(Content.builder().role("model").parts(List.of(Part.fromText("alpha"))).build()).build();
    final Event second = Event.builder().id(null).invocationId("run-2").turnComplete(true)
        .finishReason(new FinishReason(FinishReason.Known.STOP))
        .content(Content.builder().role("model").parts(List.of(Part.fromText("beta"))).build()).build();

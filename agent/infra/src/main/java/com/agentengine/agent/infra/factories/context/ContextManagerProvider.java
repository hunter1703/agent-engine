...
public class ContextManagerProvider {
    ...
    public ContextManagerProvider(
            final Instance<ContextManagerFactory<?, ?>> allFactories,
            final CompactionContextManagerFactory compactionContextManagerFactory) {
        this.typeVsFactory = CollectionUtils.transformToMap(
                allFactories.stream().toList(), ContextManagerFactory::type, Function.identity());
        this.defaultFactory = compactionContextManagerFactory;
    }

    @SuppressWarnings("unchecked")
    public <C extends ContextStrategyConfig, CM extends ContextManager> ContextManager create(
            final BaseAgentConfig agentConfig) {
        final C resolvedConfig = (C) AgentUtils.resolveContextStrategy(agentConfig);
        // noinspection unchecked
        final ContextManagerFactory<C, CM> factory =
                (ContextManagerFactory<C, CM>) typeVsFactory.getOrDefault(resolvedConfig.getType(), defaultFactory);

...
public final class AdjustExposureTool extends ImageEditingTool {
  ...
  private static File applyExposure(final File inputFile, final ExposureAdjustment adj)
      throws IOException {
    ...
    final float[] baseLayer =
        (adj.shadows() != 0 || adj.highlights() != 0) ? ImageUtils.computeBaseLayer(image) : null;

    final int w = image.getWidth();
    // Apply per-tile, passing the base layer slice for each tile's absolute pixel positions
    ImageUtils.processTiledInPlace(
        image,
        (pixels, tileX, tileY, tileW, tileH) -> {

 ...
 *
 * <p>When the model produces output that violates a protocol (e.g., tool calls in partial
 * responses), a violation is recorded with a correction message. This processor retrieves those
 * violations and appends their correction messages as user content to the next request, providing
 * the model with explicit feedback on how to correct its behavior.
 *
 * <h3>Guarantees</h3>
 *
 * <ul>
 *   <li>All accumulated violations are converted into correction content and appended to the
 *       request.
 *   <li>After violations are processed, the violation list is cleared for the next turn.
 *   <li>If no violations are present, the request passes through unchanged.
 *   <li>Correction content is added as user messages (role="user") to the request.
 * </ul>
 *

...
/**
 * Agent tool that adjusts clarity — negative values create a dreamy glow effect, positive values
 * increase local contrast for a crisp, sharp look.
 *
 * <p>Unlike the other image tools, clarity operates on the full image (not tiled) because the
 * Gaussian blur it uses requires spatial context across tile boundaries.
 */
@DiscoverableTool
public final class AdjustClarityTool extends ImageEditingTool {
  ...
  public ToolOutput<Map<String, Object>> execute(
    ...
    if (clarity == 0.0) {
      return ToolOutput.direct(Map.of("artifactName", artifactName));
    }
    return processImage(
        artifactName,
        inputFile -> {

...
/**
 * Vignette: darkens or lightens image edges with an elliptical cosine falloff, with optional
 * saturation adjustment at the edges. Matches Lightroom's Effects panel XMP fields {@code
 * crs:VignetteAmount} and {@code crs:VignetteMidpoint}. The saturation parameter is inspired by
 * darktable's vignette module (GPL), which independently controls edge saturation.
 *
    ...
    @ToolSchema(
            ...
            description =
                "Saturation change applied at the vignette edges, integer [-100, 100]. "
                    ...
                    + "Only applied where the vignette effect is active (outside the midpoint radius).",
            optional = true)

...
public final class ImageUtils {
        ...
        for (final Rectangle tile : tiles) {
            callables.add(() -> {
                ...
                for (int row = 0; row < tile.height; row++) {
                    System.arraycopy(
                            destPixels, (tile.y + row) * width + tile.x, tilePixels, row * tile.width, tile.width);
                }
                tileOp.apply(tilePixels, tile.x, tile.y, tile.width, tile.height);
                for (int row = 0; row < tile.height; row++) {
                    System.arraycopy(
                            tilePixels, row * tile.width, destPixels, (tile.y + row) * width + tile.x, tile.width);
                }
                return null;
            });
        }
            ...
            for (final Rectangle tile : tiles) {
                callables.add(() -> {
                    ...
                    try (final ImageInputStream stream = ImageIO.createImageInputStream(inputFile)) {
                        ...
                        for (int row = 0; row < tile.height; row++) {
                            System.arraycopy(
                                    pixels, row * tile.width, destPixels, (tile.y + row) * width + tile.x, tile.width);
                        }
                        return null;
                    } finally {
                        reader.dispose();
    ...
    public static float[] computeBaseLayer(final BufferedImage image) {
        ...
        for (int sy = 0; sy < smallH; sy++) {
            for (int sx = 0; sx < smallW; sx++) {
                ...
                small[sy * smallW + sx] = (float) (lum[y0 * w + x0] * (1 - wx) * (1 - wy)
                        + lum[y0 * w + x1] * wx * (1 - wy)
                        + lum[y1 * w + x0] * (1 - wx) * wy
                        + lum[y1 * w + x1] * wx * wy);
            }
        ...
        for (int fy = 0; fy < h; fy++) {
            for (int fx = 0; fx < w; fx++) {
                ...
                base[fy * w + fx] = (float) (blurredSmall[y0 * smallW + x0] * (1 - wx) * (1 - wy)
    ...
    private static float[] downsampleLinear(final float[] src, final int sw, final int sh, final int dw, final int dh) {
        ...
        for (int dy = 0; dy < dh; dy++) {
            for (int dx = 0; dx < dw; dx++) {
                ...
                final double wx = fx - x0, wy = fy - y0;
                for (int c = 0; c < 3; c++) {
                    dst[(dy * dw + dx) * 3 + c] = (float) (src[(y0 * sw + x0) * 3 + c] * (1 - wx) * (1 - wy)
    ...
    private static float[] upsampleLinear(final float[] src, final int sw, final int sh, final int dw, final int dh) {

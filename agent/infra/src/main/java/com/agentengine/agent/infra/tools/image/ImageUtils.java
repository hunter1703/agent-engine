 ...
 * sRGB-encoded pixels are decoded to linear before any adjustment and re-encoded afterward. This is
 * the only physically correct approach: gamma-space arithmetic produces wrong exposure scaling,
 * wrong tone curves, wrong saturation, and wrong vignette darkening.
 *
 * <p>Algorithm sources:
 *
 * <ul>
 *   <li>sRGB ↔ linear: IEC 61966-2-1 (exact piecewise formula)
 *   <li>Exposure: linear EV scale pow(2, ev) in linear light
 *   <li>Tone curve: monotone cubic Hermite spline in linear light, ±100 → ±0.25 zone shift
 *   <li>Temperature: Kang et al. (2002) Planckian locus → CIE xy → von Kries LMS adaptation → sRGB
 *   <li>Saturation: sRGB → linear → Oklab → chroma scale → linear → sRGB
 *   <li>HSL color mix: hue-targeted adjustments in HSL (matches Lightroom Color Mixer)
 *   <li>Split toning: luminance-weighted hue tinting in HSL, zone weights from linear luminance
 *   <li>Vignette: elliptical cosine falloff applied in linear light
 *   <li>Clarity: Gaussian blur + unsharp mask / screen blend in linear light
 * </ul>
 */
public final class ImageUtils {
  ...
  /**
   * sRGB → linear decode LUT: SRGB_TO_LINEAR[i] = linear value for sRGB byte i. IEC 61966-2-1:
   * linear = (srgb/255)^2.4 / 1.055 + 0.055/1.055 for srgb > 0.04045*255, else linear = (srgb/255)
   * / 12.92.
   */
  private static final double[] SRGB_TO_LINEAR = new double[256];

  /**
   * Linear → sRGB encode LUT: LINEAR_TO_SRGB[i] maps linear*255 index to sRGB byte. Sampled at 4096
   * points for sub-byte accuracy, then rounded.
   */
  ...
  /**
   * Pixel operation that receives both the pixel array and the tile's position within the full
   * image. Use this instead of {@link Consumer} when the operation needs to know absolute pixel
   * coordinates (e.g. vignette, gradient masks).
   */
  @FunctionalInterface
  ...
  /**
   * Process an image file tile-by-tile, applying {@code pixelOp} to each tile's pixel array in
   * place. Use this overload when the operation does not need tile coordinates.
   */
  ...
  public static void processTiledInPlace(
      final BufferedImage image, final InPlaceTileOperation tileOp) {
  ...
  /**
   * Process an image file tile-by-tile, applying {@code tileOp} to each tile with full coordinate
   * context. Use this overload when the operation needs to know each pixel's absolute position
   * (e.g. vignette).
   */
        ...
        callables.add(
            () -> {
              ...
              try (final ImageInputStream stream = ImageIO.createImageInputStream(inputFile)) {
                ...
                final int[] pixels =
                    ((DataBufferInt) normalized.getRaster().getDataBuffer()).getData();
  ...
  /**
   * Write a {@link BufferedImage} to a file at high quality. JPEG is written at {@value
   * JPEG_OUTPUT_QUALITY} quality to prevent generational loss on repeated edits. PNG is lossless
   * and written directly.
   */
  static void writeImage(final BufferedImage image, final String format, final File outputFile)
      throws IOException {
  ...
  private static BufferedImage readAndNormalizeTile(final ImageReader reader, final Rectangle tile)
      throws IOException {
  ...
  /**
   * Derive the canonical format string from a filename. Accepts jpg/jpeg → {@code "jpeg"}, png →
   * {@code "png"}; throws for anything else.
   */
  ...
  /**
   * Detect image format from the first bytes of a stream. Recognises JPEG ({@code FF D8 FF}) and
   * PNG ({@code 89 PNG}).
   */
  ...
  /**
   * Build a cosine falloff lookup table for the given hue width. Entry {@code i} holds {@code
   * cos((i / hueWidth) * (π/2))} for i ≤ hueWidth, else 0.
   *
   ...
   *
   * <p>Decodes sRGB → linear → HSL, applies hue/saturation/luminance deltas weighted by a cosine
   * falloff around the target hue, then converts back HSL → linear → sRGB. Operating through linear
   * light ensures the HSL conversion sees perceptually correct luminance values rather than
   * gamma-compressed ones.
   *
   * <p>Matches Lightroom's Color Mixer (HSL panel) semantics: hue_shift in degrees, sat_delta and
   * lum_delta on [-100, 100].
   */
  public static void applyColorAdjustments(
      final int[] pixels, final List<ColorAdjustment> adjustments, final List<double[]> cosLuts) {
   ...
   *
   * <ol>
   *   <li>Decode sRGB → linear (IEC 61966-2-1)
   *   <li>EV exposure: multiply by {@code pow(2, ev)}
   *   <li>Blacks/Whites: global endpoint tone curve (monotone cubic Hermite spline)
   *   <li>Shadows/Highlights: local-adaptation blend using a pre-computed Gaussian base layer
   *   <li>Saturation: convert to Oklab, scale chroma, convert back
   *   <li>Encode linear → sRGB
   * </ol>
   *
   * <p>Shadows and Highlights use the darktable/Lightroom local-adaptation technique: a
   * Gaussian-blurred base layer is inverted and blended back via an overlay blend, weighted by a
   * luminance zone mask. This preserves local contrast (edges, texture) while adjusting global
   * tonal zones — matching Lightroom PV2012 behaviour. Reference: darktable shadhi.c (GPL), He et
   * al. Guided Image Filtering (2013).
   *
   * @param pixels TYPE_INT_RGB packed ints, modified in place
   * @param adj exposure parameters
   * @param baseLayer pre-computed Gaussian base layer (linear luminance per pixel, same length as
   *     pixels); pass null if shadows and highlights are both 0
   */
  public static void applyExposureAdjustments(
      final int[] pixels, final ExposureAdjustment adj, final float[] baseLayer) {
      ...
      if ((hasShadows || hasHighlights) && baseLayer != null) {
        ...
        if (hasHighlights) {
          ...
          while (hs2 > 0.0) {
            ...
            final double lbH =
                (lb - 0.5) * Math.signum(-highlightsStrength) * Math.signum(1.0 - laH) + 0.5;
        ...
        if (hasShadows) {
          ...
          final double shadowsXform =
              Math.clamp(lb / (1.0 - compress) - compress / (1.0 - compress), 0.0, 1.0);
          ...
          while (ss2 > 0.0) {
            ...
            ss2 -= 1.0;
            final double lbS =
                (lb - 0.5) * Math.signum(shadowsStrength) * Math.signum(1.0 - laS) + 0.5;
  ...
  /**
   * Overlay/soft-light blend: the core of darktable's shadows/highlights algorithm. For la ≤ 0.5:
   * result = 2 * la * lb For la > 0.5: result = 1 - 2 * (1 - la) * (1 - lb) Blended with the
   * original by {@code optrans}.
   */
  private static double overlayBlend(final double la, final double lb, final double optrans) {
    final double blended = (la > 0.5) ? 1.0 - (1.0 - 2.0 * (la - 0.5)) * (1.0 - lb) : 2.0 * la * lb;
    return la * (1.0 - optrans) + blended * optrans;
  }

  /**
   * Build a 4097-entry tone curve LUT for Blacks and Whites endpoint controls only. Shadows and
   * Highlights are handled separately via local adaptation.
   */
  private static double[] buildEndpointLut(final ExposureAdjustment adj) {
    ...
    for (int i = 0; i < n - 1; i++) {
      if (delta[i] == 0.0) {
        ...
        if (tau > 9.0) {
          final double s = 3.0 / Math.sqrt(tau);
          m[i] = s * alpha * delta[i];
   ...
   *
   * <p>The base layer is the linear CIE Y luminance of each pixel after a large-radius Gaussian
   * blur. It captures the large-scale tonal structure of the image without fine detail. The
   * shadows/highlights blend uses this to determine how much each pixel should be adjusted: dark
   * pixels in the base layer get the shadow treatment, bright pixels get the highlight treatment.
   *
   * <p>The blur radius is fixed at 10% of the shorter image dimension, matching darktable's default
   * radius of 100px on a typical image. This is large enough to capture global tonal zones while
   * remaining fast via the half-resolution trick.
   *
  ...
  private static float[] gaussianBlurFloat(
      final float[] src, final int w, final int h, final double sigma) {
   ...
   *
   * <ol>
   *   <li>Convert target Kelvin to CIE xy chromaticity using the Kang et al. (2002) formula (exact
   *       Planckian locus fit, valid 1667K–25000K)
   *   <li>Apply tint as a perpendicular shift on the Planckian locus in CIE uv space
   *   <li>Compute von Kries chromatic adaptation from D65 (the sRGB white point) to the target
   *       illuminant in CAT02 LMS space
   *   <li>Convert the resulting 3×3 adaptation matrix to linear sRGB channel multipliers
   *   <li>Decode sRGB → linear, apply multipliers, encode linear → sRGB
   * </ol>
   *
   * <p>This is the same pipeline used by dcraw, RawTherapee, and darktable for rendered images.
   */
  public static void applyTemperatureAdjustment(
      final int[] pixels, final TemperatureAdjustment adj) {
   ...
   *
   * <p>The full pipeline is: linear sRGB → XYZ (D65) → CAT02 LMS → diagonal gain → CAT02⁻¹ LMS →
   * XYZ (D65) → linear sRGB. Pre-multiplied into a single 3×3 matrix.
   *
   ...
   */
  private static double[] adaptedSrgbRow(
      final int row, final double gainL, final double gainM, final double gainS) {
   ...
   *
   * <p>Zone weights are computed from the pixel's linear luminance (CIE Y), which is perceptually
   * correct — gamma-encoded L would compress the shadow region and expand highlights, making the
   * crossover point feel wrong.
   *
   * <p>The tinting itself operates in HSL space (hue blend + saturation introduction), which
   * matches Lightroom's Color Grading panel behaviour.
   */
   ...
   *
   * <p>Saturation algorithm from darktable vignette.c (GPL): {@code col[c] = col[c] - (mean -
   * col[c]) * weight * saturation} where mean = (R+G+B)/3. Negative saturation pushes channels
   * toward the mean (desaturates); positive pushes away (saturates).
   */
  public static void applyVignette(
    ...
    final double featherWidth =
        Math.max(0.01, (adj.feather() <= 0 ? 50.0 : adj.feather()) / 100.0) * 0.5;
   ...
   * incorrect halos because the gamma curve compresses bright regions — a bright edge next to a
   * dark region blurs to a value that is too dark in linear terms, causing the unsharp mask to
   * over-sharpen bright edges and under-sharpen dark ones.
   *
   * <p>Positive clarity: unsharp mask (local contrast boost). Negative clarity: screen blend of
   * blurred image (dreamy glow / mist).
   */
  ...
  private static float[] downsampleLinear(
      final float[] src, final int sw, final int sh, final int dw, final int dh) {
  ...
  /**
   * RGB → HSL. Input channels in [0,1], output h ∈ [0,360), s and l ∈ [0,1]. Used for gamma-encoded
   * sRGB values in color adjustments (HSL is a perceptual model and must operate in the gamma
   * domain, not linear light).
   */
  ...
  /**
   * Linear sRGB → Oklab (Björn Ottosson, 2020). Returns [L, a, b] where L ∈ [0,1], a and b ∈ [-0.5,
   * 0.5] approximately. Oklab is perceptually uniform: equal Euclidean distances correspond to
   * equal perceived colour differences, and chroma scaling does not shift hue.
   */
  ...
  /**
   * Oklab → linear sRGB (Björn Ottosson, 2020). Returns linear RGB; caller must clamp to [0,1]
   * before encoding.
   */

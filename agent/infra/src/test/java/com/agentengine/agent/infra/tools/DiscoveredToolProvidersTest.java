...
class DiscoveredToolProvidersTest {
  ...
  @Test
  void shouldPopulateConfigsSchemaFromConstructorAnnotationsWhenToolIsDiscoverable()
      throws NoSuchMethodException {
    final Constructor<EchoTool> constructor = EchoTool.class.getDeclaredConstructor(String.class);
    final java.util.List<ToolParameter> params =
        DiscoveredToolProviders.resolveParameters(constructor);
    ...
    assertThat(prefix.get("description").toString())
        .contains("Prefix to add to the echoed message");
    assertThat(prefix.get("description").toString()).contains("Optional.");
  }

  @Test
  void shouldKeepExistingConfigsSchemaWhenDiscoverableToolConstructorHasNoParameters()
      throws NoSuchMethodException {
    ...
    final ToolDescriptor descriptor =
        DiscoveredToolProviders.getDescriptor(ReadFileTool.class, params);

 ...
 */
@Singleton
public class SessionHistoryServiceImpl extends AbstractJournalReadRepository
    implements SessionHistoryService {
  ...
  public List<Event> getEvents(final String sessionId) {
    ...
    for (final Object fact :
        getJournalEvents(persistenceId).stream().map(EventEnvelope::event).toList()) {
  ...
  public List<SessionEvent> getAllSessionEvents(final String sessionId) {
    ...
    final List<AgentSession> childSessions =
        sessions
            .find(
                or(
                    eq(AgentSession.FIELD_ROOT_SESSION_ID, sessionId),
                    eq(BaseEntity.FIELD_ID, sessionId)))
            .into(new ArrayList<>());
            ...
            .thenComparing(
                ...
                (one, two) -> {
                  if (Objects.equals(rootSessionId, one) && !Objects.equals(rootSessionId, two)) {
                    return -1;
                  } else if (Objects.equals(rootSessionId, two)
                      && !Objects.equals(rootSessionId, one)) {
                    return 1;
                  } else {
  ...
  private AgentSession findSession(final String sessionId) {
    ...
    final AgentSession session = sessions.find(eq(MongoUtils.FIELD_MONGO_ID, sessionId)).first();
   ...
   *
   * <p>Facts are processed sequentially: each {@link TurnCommittedFact} contributes its ADK events
   * (assigned monotonically increasing sequence numbers), and each {@link CompletedFact} with a
   * non-blank error injects a {@link SessionEvent#error} immediately after the turn that failed.
   * This correctly handles sessions with multiple runs where more than one run failed.
   */
    ...
    for (final Object fact : facts) {
      if (fact instanceof TurnCommittedFact turnCommitedFact) {
        ...
        LOG.info(
            ...
            completed.getError());
        sessionEvents.add(
            SessionEvent.error(rootSessionId, sessionId, completed.getError(), seq++));
      }
    }

    LOG.info(
        "[USER_MESSAGE_TRACE][{}] scanJournal() produced {} session events",
        sessionId,
        sessionEvents.size());

...
## Session Rollback: Turn Filtering Is Inconsistent and the Journal-Fold Loop Is Unsafe

Found while reviewing the "change event store from postgres backed to mongo backed" commit.

`RollbackFact` accounting (discarding a run's committed turns, and everything committed after
them) is only applied in one place: the private `getEvents()` helper in
`agent/core/src/main/java/com/agentengine/agent/core/factories/RunnerFactory.java`, used to rebuild
the ADK in-memory `Session` the runner operates on. Every other path that reads committed session
history goes through `SessionActorJournal.getCommittedTurnIds(sessionId)`
(`agent/core/.../session/SessionActorJournal.java`), which returns every `TurnCommittedFact` ever
persisted with **no** `RollbackFact` filtering — consumed by `MemoryService.addSessionToMemory`,
`SessionTitleGenerator.generateTitle`, and `RuntimeServiceImpl.getCommittedTurnIds` (which backs
the REST-facing history/subscribe endpoints and `SessionServiceImpl`'s root-session detail view).
`SessionServiceImpl.getSessionEvents` (the non-root/child-session path) is looser still — it does
no turn filtering at all, just fetches every stored `SessionEvent` for the session id. Net effect:
rolling back a run makes it disappear from what the agent sees on its next turn, but the rolled-back
turn's events still get ingested into memory, still influence the generated session title, and are
still returned by the REST history/subscribe API — an inconsistent view of "what happened"
depending on which code path answers.

Separately, the fold loop in `RunnerFactory.getEvents()` itself (the `while (true)` loop trimming
`survivingTurns` against each `RollbackFact`) is unsafe. It calls `survivingTurns.removeLast()`
unconditionally on every iteration, even before confirming the current tail turn belongs to the run
being rolled back:
...
`SessionActor.rollback()` only rejects a rollback when `sessionState() == RUNNING` — a client can
roll back a run that is currently `PAUSED` with zero turns committed yet, or send a stale/incorrect
`runId` that matches nothing in the journal. In either case, the loop still deletes the last
committed turn on its very first iteration regardless of whether it matched, then keeps deleting one
turn per iteration until it either accidentally matches something later (silently discarding real
conversation history unrelated to the rollback) or `survivingTurns` runs empty, at which point the
next `.getLast()` throws `NoSuchElementException` and crashes the ADK in-memory session rebuild for
that session.

Proposed direction: extract the fold logic into one shared helper (e.g. on `SessionActorJournal`)
that computes the effective/surviving committed-turn IDs for a session from its
`TurnCommittedFact`/`RollbackFact` history, fix the loop so a turn is only removed after a match is
confirmed (explicitly no-op when the target run isn't found in the journal, rather than trimming
blindly), and have every read path — `MemoryService`, `SessionTitleGenerator`,
`RuntimeServiceImpl`, and `SessionServiceImpl` for both root and child sessions — call that one
helper instead of the unfiltered `SessionActorJournal.getCommittedTurnIds`.


## ToolAssetHandler: Pagination Silently Returns Empty List for a Query Without a Page

Found while reviewing the "change event store from postgres backed to mongo backed" commit.

`interfaces/rest/src/main/java/com/agentengine/interfaces/rest/handlers/catalog/ToolAssetHandler.java`,
`findAssets()`:
...
Since that commit, `Query.page` defaults to `Page.UNBOUNDED` (offset 0, limit 0) instead of `null`.
`limit == 0` means "no limit" specifically to MongoDB's own driver (`.limit(0)` is unbounded in the
Mongo Java driver), and `QdrantVectorStore.findBySemanticQueryInternal` explicitly special-cases it
the same way (`page.getLimit() <= 0 ? Integer.MAX_VALUE : page.getLimit()`). `ToolAssetHandler`
doesn't: it slices an in-memory list with plain `tools.subList(start, end)`, and with `limit == 0`,
`end == start`, so it returns an empty list.

Regression: before this commit, `request.getQuery() != null && request.getQuery().getPage() != null`
correctly fell back to `new Page(0, 100)` whenever the caller's `Query` had no explicit page. Now
any client that sends a `Query` with a filter but no page gets zero tools back instead of up to 100.

Fix direction: special-case `limit <= 0` as "unbounded" in `ToolAssetHandler`, the same way
`QdrantVectorStore` already does.

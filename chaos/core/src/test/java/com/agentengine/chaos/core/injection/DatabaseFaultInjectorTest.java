...
class DatabaseFaultInjectorTest {
    ...
    void setUp() {
        ...
        injector = new DatabaseFaultInjector(
    ...
    void shouldAddFullBlockTimeoutToxicOnQdrantWhenLatencyAbsent()
            throws ExecutionException, InterruptedException, IOException {
        final String faultId = injector.injectFault(
                        FaultType.QDRANT_FAILURE, TARGET, new QdrantFailureParameters(Optional.empty()), BLAST_RADIUS)
                .toCompletableFuture()
                .get();
    ...
    @Test
    void shouldRemoveToxicFromTheProxyItWasAddedTo() throws ExecutionException, InterruptedException, IOException {
        final String faultId = injector.injectFault(
                        FaultType.QDRANT_FAILURE, TARGET, new QdrantFailureParameters(Optional.empty()), BLAST_RADIUS)
                .toCompletableFuture()
                .get();
    ...
    @Test
    void shouldNotThrowWhenRemovingUnknownFaultId() throws ExecutionException, InterruptedException, IOException {
    ...
    @Test
    void shouldTrackDistinctToxicsPerFaultIndependently() throws ExecutionException, InterruptedException, IOException {
        final String firstFaultId = injector.injectFault(
                        FaultType.QDRANT_FAILURE,
                        TARGET,
                ...
                .toCompletableFuture()
                .get();
        final String secondFaultId = injector.injectFault(
                        FaultType.SNAPSHOT_STORE_FAILURE,
                        TARGET,
                        new DatabaseFailureParameters(DatabaseTarget.UNKNOWN),
                        BLAST_RADIUS)
    ...
    void shouldThrowIllegalArgumentForUnsupportedFaultType() {
        final var future = injector.injectFault(
                        FaultType.POD_KILL,
                        TARGET,
                        new DatabaseFailureParameters(DatabaseTarget.POSTGRESQL),
                        BLAST_RADIUS)

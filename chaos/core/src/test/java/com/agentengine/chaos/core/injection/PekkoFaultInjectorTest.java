...
class PekkoFaultInjectorTest {
    ...
    @Test
    void shouldRegisterDelayConfigForTargetedEntity() throws ExecutionException, InterruptedException {
        final TargetSelector target = new TargetSelector("agent-engine", "agent", Map.of(), Optional.of("session-42"));
        ...
        assertThat(registry.configFor("session-42").get().delayDuration()).contains(Duration.ofSeconds(2));
    }

    @Test
    void shouldRegisterDropConfigForWildcardWhenNoEntityIdSpecified() throws ExecutionException, InterruptedException {
        final TargetSelector target = new TargetSelector("agent-engine", "agent", Map.of(), Optional.empty());
    ...
    void shouldRemoveFaultFromRegistry() throws ExecutionException, InterruptedException {
        final TargetSelector target = new TargetSelector("agent-engine", "agent", Map.of(), Optional.of("session-1"));
        final String faultId = injector.injectFault(

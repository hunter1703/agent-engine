...
class SuccessCriteriaEvaluatorTest {
    ...
    void shouldPassMaxErrorRateWhenBelowThreshold() {
        final EvaluationResult result = evaluate(
    ...
    void shouldPassMinSuccessRateWhenAboveThreshold() {
        final EvaluationResult result = evaluate(
                List.of(new SuccessCriterion(CriterionType.MIN_SUCCESS_RATE, 0.5, "graceful degradation")),
                metrics(0.6, 100, 0.4),
                Optional.empty(),
                false);
        assertThat(result.passed()).isTrue();
    ...
    private EvaluationResult evaluate(
        ...
        return evaluator.evaluate(criteria, baseline, List.of(baseline), postRecovery, recoveryTime, dataLossDetected);
    }

    private static SteadyStateMetrics metrics(final double successRate, final long p99Millis, final double errorRate) {

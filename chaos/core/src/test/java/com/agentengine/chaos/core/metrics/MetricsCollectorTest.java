...
class MetricsCollectorTest {
  ...
  void setUp() throws IOException {
    ...
    server.start();
    prometheusClient =
        new PrometheusClient(new OkHttpClient(), server.url("/").toString().replaceAll("/$", ""));
    collector =
        new MetricsCollector(
            prometheusClient,
            () -> OptionalInt.of(7),
            MetricsQueries.defaults(),
            new SteadyStateAnalyzer());
  }
  ...
  void shouldReturnEmptyWhenAllQueriesFailAndHealthEndpointHasNoData() {
    ...
    final MetricsCollector collectorWithoutHealth =
        new MetricsCollector(
            prometheusClient,
            OptionalInt::empty,
            MetricsQueries.defaults(),

...
class DatabaseFaultInjectorIT {
  ...
  @Test
  void shouldBlockAndThenRestoreConnectionsThroughTheProxy()
  ...
  /**
   * Connects through the Toxiproxy-proxied port, sends a byte, and waits for any response. A plain
   * TCP {@code connect()} alone is not enough to detect a {@code timeout} toxic — Toxiproxy still
   * completes the handshake and simply stops forwarding bytes afterward — so this checks that data
    ...
    try (Socket socket = new Socket()) {
      socket.connect(
          new InetSocketAddress(
              postgresProxy.getContainerIpAddress(), postgresProxy.getProxyPort()),
          1_000);

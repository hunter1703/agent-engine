...
/**
 * Pre-flight safety check run before any fault is injected. Only meaningful for pod-targeted
 * faults (Chaos Mesh infrastructure faults) — experiments with no {@code podLabels} target an
 * entity ID, database proxy, or LLM endpoint instead of a set of pods, so there is nothing to
 * count and the check passes trivially.
 */
public final class BlastRadiusEnforcer {
    ...
    public BlastRadiusDecision enforce(final TargetSelector target, final BlastRadius blastRadius) {
        ...
        if (matchingPods == 0) {
            return BlastRadiusDecision.rejected("No pods matched target selector " + target.podLabels(), 0);
        }

...
public final class ExperimentReportGenerator {
  ...
  public String toMarkdown(final ExperimentResult result) {
    ...
    result
        .recoveryTime()
        .ifPresent(duration -> md.append("- **Recovery time**: ").append(duration).append('\n'));
    result
        .abortReason()
        .ifPresent(reason -> md.append("- **Abort reason**: ").append(reason).append('\n'));
    md.append('\n');
  ...
  public String toHtml(final ExperimentResult result) {
    ...
    result
        .recoveryTime()
        .ifPresent(
            duration -> html.append("<p><b>Recovery time:</b> ").append(duration).append("</p>"));

    html.append(
        "<h2>Steady State</h2><table border=\"1\"><tr><th>Metric</th><th>Baseline</th>"
            + "<th>Post-Recovery</th></tr>");
    if (result.baseline().isPresent() && result.postRecovery().isPresent()) {
      appendMetricsRowsHtml(html, result.baseline().get(), result.postRecovery().get());
    }
    html.append("</table>");
        ...
        .ifPresent(
            evaluation -> {
              ...
              if (evaluation.failures().isEmpty()) {
                ...
                html.append(
                    "<table border=\"1\"><tr><th>Criterion</th><th>Threshold</th><th>Actual</th>"

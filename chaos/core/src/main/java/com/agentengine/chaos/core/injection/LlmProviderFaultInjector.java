 ...
 * embedded (integration test) or remote (staging) WireMock instance. In staging, the same endpoints
 * are expected to sit behind a Toxiproxy proxy instead — per design.md, this class covers the
 * WireMock delivery mechanism only.
 *
 ...
 * syntax passed to {@link WireMock#urlMatching(String)}) identifying which requests the stub should
 * intercept, e.g. {@code /v1/chat/completions} for an LLM provider or {@code /connectors/.*} for an
 * outbound connector. {@code namespace} and {@code podLabels} are ignored by this injector (they
 * are meaningful only to the Kubernetes-backed injectors); {@code entityId} is unused here since a
 * stub mapping is not scoped to a single session.
 */
public final class LlmProviderFaultInjector implements FaultInjector {

  /**
   * Status returned by {@link FaultType#LLM_PROVIDER_LATENCY} stubs: the call still succeeds, just
   * slowly.
   */
  ...
  private static ResponseDefinitionBuilder responseFor(
    ...
    return switch (faultType) {
      ...
      case LLM_PROVIDER_LATENCY -> {
        final LlmProviderLatencyParameters latencyParameters =
            (LlmProviderLatencyParameters) parameters;
        yield aResponse()
            .withStatus(LATENCY_RESPONSE_STATUS)
            .withFixedDelay((int) latencyParameters.latency().toMillis());
      }
      case CONNECTOR_FAILURE -> aResponse().withFault(Fault.CONNECTION_RESET_BY_PEER);
      default ->
          throw new IllegalArgumentException(
              "LlmProviderFaultInjector does not support " + faultType);
    };
  }
}

...
/**
 * Injects infrastructure-level faults ({@link FaultType#POD_KILL}, network, stress, and I/O faults)
 * by creating Chaos Mesh custom resources via the Fabric8 {@link GenericKubernetesResource} API.
 * Chaos Mesh's own controller drives the fault lifecycle from there — killing pods, throttling
 * network traffic, or injecting stress — so this class only needs to create and delete the CR
 * describing the experiment; it never talks to the target pods directly.
 *
 ...
 * can delete the exact resource without a separate lookup. A 404 on delete means Chaos Mesh (or an
 * operator) already removed the resource, which is the same end state {@code removeFault} is trying
 * to reach, so it is treated as success rather than an error.
 */
public final class ChaosMeshFaultInjector implements FaultInjector {
        ...
        () -> {
          ...
          final GenericKubernetesResource resource =
              new GenericKubernetesResourceBuilder()
                  ...
                  .build();
          client
              .genericKubernetesResources(resourceDefinitionFor(kind))
  ...
  private static String kindFor(final FaultType faultType) {
    return switch (faultType) {
      ...
      case NETWORK_PARTITION, NETWORK_LATENCY, NETWORK_PACKET_LOSS, CLUSTER_PARTITION ->
          "NetworkChaos";
      case CPU_STRESS, MEMORY_STRESS -> "StressChaos";
      case DISK_STRESS -> "IOChaos";
      default ->
          throw new IllegalArgumentException(
              "ChaosMeshFaultInjector does not support " + faultType);
    };
  }

  /**
   * Chaos Mesh CRD plurals are simply the kind lowercased, e.g. {@code PodChaos} → {@code
   * podchaos}.
   */
  ...
  private static Map<String, Object> specFor(
    ...
    return switch (faultType) {
      ...
      case NETWORK_PARTITION ->
          networkPartitionSpec(target, (NetworkPartitionParameters) parameters);
  ...
  private static Map<String, Object> podKillSpec(
      final TargetSelector target, final PodKillParameters parameters) {
    return Map.of(
        "action",
        "pod-kill",
        "mode",
  ...
  private static Map<String, Object> networkPartitionSpec(
    ...
    return Map.of(
        ...
        "target",
            Map.of("selector", Map.of("services", parameters.blockedServices()), "mode", "all"));
  }

...
/**
 * Injects {@link FaultType#DATABASE_FAILURE}, {@link FaultType#EVENT_JOURNAL_FAILURE},
 * {@link FaultType#SNAPSHOT_STORE_FAILURE}, and {@link FaultType#QDRANT_FAILURE} faults as
 * Toxiproxy toxics on the {@code postgresql}, {@code mongodb}, and {@code qdrant} proxies. Each
 * injected toxic is named uniquely so {@link #removeFault} can locate and delete it without the
 * caller needing to remember which proxy it lives on.
 */
public final class DatabaseFaultInjector implements FaultInjector {

    private static final long FULL_BLOCK_TIMEOUT_MILLIS = 0L;
    private static final long SNAPSHOT_STORE_LATENCY_MILLIS =
            ...
            try {
                return switch (faultType) {
                    case DATABASE_FAILURE ->
                        injectFullBlock(
                                faultType,
                                proxyFor(((DatabaseFailureParameters) parameters)
                                        .target()
                                        .name()
                                        .toLowerCase()));
                    ...
                    case QDRANT_FAILURE -> injectQdrantFault(faultType, (QdrantFailureParameters) parameters);
                    default ->
                        throw new IllegalArgumentException("DatabaseFaultInjector does not support " + faultType);
                };
    ...
    private String injectQdrantFault(final FaultType faultType, final QdrantFailureParameters parameters)
            throws IOException {
    ...
    private String injectLatency(final FaultType faultType, final Proxy proxy, final long latencyMillis) {
        return addToxic(faultType, proxy, "latency", latencyMillis);
    }

    private String addToxic(final FaultType faultType, final Proxy proxy, final String toxicKind, final long millis) {
        ...
        try {
            ...
            throw new FaultInjectionException("Failed to add " + toxicKind + " toxic for " + faultType, ex);
        }
    ...
    /** Wraps checked Toxiproxy client failures so they can propagate through a {@link CompletionStage}. */

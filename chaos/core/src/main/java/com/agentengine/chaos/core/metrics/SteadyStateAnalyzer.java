...
/**
 * Aggregates raw {@link SteadyStateMetrics} snapshots into a single baseline/post-recovery reading,
 * and determines how far a snapshot has deviated from baseline.
 *
...
public final class SteadyStateAnalyzer {
  ...
  public double deviationPercentage(
      final SteadyStateMetrics baseline, final SteadyStateMetrics candidate) {
    final double successRateDeviation =
        relativeDelta(baseline.successRate(), candidate.successRate());
    final double latencyDeviation =
        relativeDelta(baseline.p99Latency().toNanos(), candidate.p99Latency().toNanos());

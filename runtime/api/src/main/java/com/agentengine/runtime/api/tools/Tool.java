package com.agentengine.runtime.api.tools;
...
/**
 * All tools must have one method named "execute". The method can have any
 * parameters.
 */
public abstract class Tool extends BaseTool {
  ...
  private static final Map<JavaType, Map<String, PropertyBinding>> PROPERTY_BINDINGS_CACHE = new ConcurrentHashMap<>();
  ...
  private Maybe<Map<String, Object>> call(Map<String, Object> args, ToolContext toolContext)
    ...
    return switch (result) {
      ...
      case Single<?> single -> single.map(data -> Utils.convertValue(data, new TypeReference<Map<String, Object>>() {
      })).toMaybe();
      default -> {
        try {
    ...
    for (ParameterBinding binding : parameterBindings) {
      ...
      if (!args.containsKey(binding.name())) {
        ...
        throw new IllegalArgumentException(
            String.format("The parameter '%s' was not found in the arguments provided by the model.", binding.name()));
      }
  ...
  private record ParameterBinding(int index, String name, boolean optional, Class<?> rawType, JavaType javaType, JavaType elementType,
      boolean isList, boolean isMap, boolean isToolContext, boolean isInputStream) {
  }

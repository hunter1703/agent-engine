...
class ShellCommandToolTest {
  ...
  void shouldPauseForConfirmationWhenCommandMatchesBlockedPattern() {
    final ToolContext toolContext = ToolContext.builder(null).functionCallId("call-1").build();
    ...
    final ToolConfirmation confirmation = toolContext.actions().requestedToolConfirmations().get("call-1");
  ...
  void shouldRejectCommandWhenConfirmationIsDenied() {
    final ToolContext toolContext = ToolContext.builder(null).functionCallId("call-1")
        .toolConfirmation(ToolConfirmation.builder().confirmed(false).build()).build();
  ...
  void shouldExecuteCommandWhenBlockedPatternIsConfirmed() {
    final ToolContext toolContext = ToolContext.builder(null).functionCallId("call-1")
        .toolConfirmation(ToolConfirmation.builder().confirmed(true).build()).build();

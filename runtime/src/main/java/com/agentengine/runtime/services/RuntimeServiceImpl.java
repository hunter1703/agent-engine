...
public class RuntimeServiceImpl implements RuntimeService {
  ...
  public StartSessionResult startSession(final String agentId, final String sessionId, final String message) {
    ...
    return ref.<Done>ask(replyTo -> new ExternalCommand.InitializeCommand(topology, replyTo), SessionActorFactory.ASK_TIMEOUT)
        .thenCompose(ignored -> ref.<StartSessionResult>ask(replyTo -> new ExternalCommand.StartCommand(message, replyTo),
  ...
  @Override
  public ResumeResult resumeSession(final String agentId, final String sessionId, final String confirmationId, final boolean confirmed,
      final String answer) {
    LOG.info("Resuming session {}:{} with confirmation '{}'", agentId, sessionId, confirmationId);
    final EntityRef<SessionCommand> ref = sessionActorFactory.entityRef(agentId, sessionId);
    return ref.<ResumeResult>ask(replyTo -> new ExternalCommand.ResumeCommand(confirmationId, confirmed, answer, replyTo),

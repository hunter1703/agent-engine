...
public class SessionActorFactory extends ShardedEntityFactory<SessionCommand> {
  ...
  @Inject
  public SessionActorFactory(final ActorSystem<SpawnProtocol.Command> actorSystem, final SessionEventChannel sessionEventChannel,
      final RunnerFactory runnerFactory, final MongoSessionService sessionService) {
    super(
        actorSystem, SessionActor.TYPE_KEY, entityContext -> Behaviors.setup(actorCtx -> new SessionActor(actorCtx,
            entityContext.getEntityId(), sessionEventChannel, refSupplier(actorSystem), runnerFactory, sessionService)),
        Duration.ofHours(1));
  }

  private static BiFunction<String, String, EntityRef<SessionCommand>> refSupplier(final ActorSystem<SpawnProtocol.Command> actorSystem) {

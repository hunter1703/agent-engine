...
public final class SessionRunner {
  ...
  public synchronized void start(final String message) {
    cancel();
    disposable = runner.runAsync(AgentSession.DEFAULT_USER_ID, sessionId, ContentUtils.buildUserContent(message), runConfig())
        .subscribeOn(SCHEDULER).subscribe(event -> sessionActor.tell(new PublishEventCommand(event)),
            error -> sessionActor.tell(new RunFailedCommand(errorMessage(error))));
  }

  public synchronized void resume(final String confirmationId, final Boolean confirmed, final String answer) {
    cancel();
    disposable = runner
        .runAsync(AgentSession.DEFAULT_USER_ID, sessionId,
            ContentUtils.buildConfirmationEvent(confirmationId, confirmed, answer).content().orElseThrow(), runConfig())
        .subscribeOn(SCHEDULER).subscribe(event -> sessionActor.tell(new PublishEventCommand(event)),
            error -> sessionActor.tell(new RunFailedCommand(errorMessage(error))));
  }

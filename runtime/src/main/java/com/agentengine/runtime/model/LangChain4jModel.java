...
/**
 * ADK {@link BaseLlm} implementation backed by LangChain4j's {@link ChatModel}
 * and {@link StreamingChatModel}.
 *
 * <h3>Guarantees</h3>
 *
 * <ul>
 * <li><b>Streaming responses:</b> {@code partial=true} events are content
 * deltas (text or thinking tokens). Subsequent {@code partial=false} event
 * contains the fully assembled response. Tool calls appear only in
 * {@code partial=false}, never in chunks.
 * <li><b>Thinking in responses:</b> If thinking is present from the model and
 * {@code returnThinking=true} is set on the underlying
 * {@link StreamingChatModel} or {@link ChatModel}:
 * <ul>
 * <li>Streaming: thinking tokens appear as {@code thought=true, partial=true}
 * events.
 ...
 *
 * <ul>
 * <li>Tool specifications and schemas must be well-formed (declarations and
 * parameter schemas present). Missing declarations/schemas cause
 ...
 *
 * <p>
 * When converting session history to API messages, text is dropped from model
 * turns that also contain tool calls. OpenAI-compatible APIs (e.g. llama.cpp)
 * reject {@code AssistantMessage} with both {@code content} and
 * {@code tool_calls}. The text is preserved in the session {@code Content}
...
public final class LangChain4jModel extends BaseLlm {
  ...
  public LangChain4jModel(final ChatModel chatModel, final StreamingChatModel streamingChatModel, final String modelName) {
  ...
  public Flowable<LlmResponse> generateContent(final LlmRequest llmRequest, final boolean stream) {
    if (stream) {
      ...
      return Flowable.create(emitter -> streamingChatModel.chat(toChatRequest(llmRequest), new StreamingChatResponseHandler() {
        @Override
        public void onPartialResponse(final String token) {
          emitter.onNext(
              LlmResponse.builder().content(Content.builder().role("model").parts(Part.fromText(token)).build()).partial(true).build());
        }

        @Override
        public void onPartialThinking(final PartialThinking partialThinking) {
          emitter.onNext(LlmResponse.builder()
              .content(Content.builder().role("model").parts(Part.builder().text(partialThinking.text()).thought(true).build()).build())
              .partial(true).build());
        }
      ...
      return Flowable.just(finalResponse(toParts(chatModel.chat(toChatRequest(llmRequest)).aiMessage())));
    }
  }
  ...
  private static LlmResponse finalResponse(final List<Part> parts) {
    return LlmResponse.builder().content(Content.builder().role("model").parts(parts).build()).partial(false).build();
  }
  ...
  private static List<ChatMessage> toMessages(final LlmRequest llmRequest) {
    final List<ChatMessage> messages = new ArrayList<>(llmRequest.getSystemInstructions().stream().map(SystemMessage::from).toList());
  ...
  private static List<ChatMessage> toUserOrToolResultMessages(final Content content) {
    ...
    return List.of(UserMessage.from(parts.stream().map(LangChain4jModel::toUserContent).toList()));
  }
  ...
  private static dev.langchain4j.data.message.Content toMediaContent(final Blob blob) {
    ...
    if (mimeType.startsWith("audio/")) {
      return AudioContent.from(Audio.builder().base64Data(base64).mimeType(mimeType).build());
    }
    if (mimeType.startsWith("video/")) {
      return VideoContent.from(Video.builder().base64Data(base64).mimeType(mimeType).build());
    }
    if (mimeType.startsWith("image/")) {
      return ImageContent.from(Image.builder().base64Data(base64).mimeType(mimeType).build());
    }
    if (mimeType.startsWith("application/pdf")) {
      return PdfFileContent.from(PdfFile.builder().base64Data(base64).mimeType(mimeType).build());
    }
    if (mimeType.startsWith("text/") || mimeType.equals("application/json") || mimeType.endsWith("+json") || mimeType.endsWith("+xml")) {
  ...
  private static AiMessage toAiMessage(final Content content) {
    ...
    content.parts().orElse(List.of()).forEach(part -> {
      if (part.text().isPresent()) {
        ...
        toolCalls.add(ToolExecutionRequest.builder().id(fc.id().orElseThrow()).name(fc.name().orElseThrow())
            .arguments(JsonUtils.toJson(fc.args().orElseThrow())).build());
      } else {
  ...
  private static List<Part> toParts(final AiMessage aiMessage) {
    ...
    if (aiMessage.hasToolExecutionRequests()) {
      for (final ToolExecutionRequest request : aiMessage.toolExecutionRequests()) {
        parts.add(Part.builder().functionCall(FunctionCall.builder().id(request.id() != null ? request.id() : UUID.randomUUID().toString())
            .name(request.name()).args(CollectionUtils.nullSafeMap(JsonUtils.fromJson(request.arguments(), new TypeReference<>() {
            }))).build()).build());
      }
    }
    return parts;
  ...
  private static List<ToolSpecification> toToolSpecifications(final LlmRequest llmRequest) {
    return llmRequest.tools().values().stream().map(tool -> {
      final FunctionDeclaration decl = tool.declaration().orElseThrow(() -> new IllegalStateException("Tool lacking declaration: " + tool));
      final Schema schema = decl.parameters().orElseThrow(() -> new IllegalStateException("Tool lacking parameters: " + tool));
      return ToolSpecification.builder().name(tool.name()).description(tool.description()).parameters(toParameters(schema)).build();
    }).toList();
  }
  ...
  private static JsonSchemaElement toSchemaElement(final Schema schema) {
    ...
    return switch (schema.type().get().knownEnum()) {
      ...
      case ARRAY -> JsonArraySchema.builder().description(description).items(toSchemaElement(schema.items().orElseThrow())).build();
      case OBJECT -> toParameters(schema);
      default -> throw new UnsupportedFeatureException("Unsupported schema type: " + schema.type().get());
    };
  }
}

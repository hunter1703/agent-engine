...
import com.agentengine.runtime.agents.processors.response.RunCompletionResponseProcessor;
 ...
 * that enforce protocol semantics, handle tool calls, and manage turn completion and loop
 * termination.
 *
 * <h3>Guarantees</h3>
 *
 * <ul>
 *   <li><b>Event loop termination:</b> The flow emits events until a terminal event is observed. A
 *       terminal event has {@code finishReason} set.
 *   <li><b>Tool session events:</b> When a tool sets {@code endInvocation=true}, the resulting
 *       event will have {@code finishReason=STOP} (if not already present) and {@code
 *       turnComplete=true}.
 *   <li><b>Continuation:</b> If an event is terminal but has no {@code finishReason}, the flow
 *       continues (loops again). Such events may appear in session history but are marked as
 *       internal (implementation detail for session tracking).
 *   <li><b>Turn limit:</b> If configured with a turn limit, the flow halts with {@code
 *       finishReason=STOP} after that many turns.
 * </ul>
 */
public final class BaseFlow extends SingleFlow {
    ...
    public BaseFlow(final Integer maxSteps) {
        super(
                REQUEST_PROCESSORS,
                CollectionUtils.append(RESPONSE_PROCESSORS, new RunCompletionResponseProcessor(maxSteps)),
     ...
     *
     * <p>The flow emits events in sequence. When a terminal event (one with {@code finishReason} set)
     * is observed, the loop terminates. Tool session events with {@code endInvocation=true} are
     * normalized to include {@code finishReason=STOP} and {@code turnComplete=true}.
     *
     * <p>Terminal events without {@code finishReason} (used to signal continuation) are marked
     * internal so they remain in session history but are excluded from user-facing output.
     */

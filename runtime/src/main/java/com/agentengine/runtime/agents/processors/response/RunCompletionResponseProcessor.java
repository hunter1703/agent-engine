 ...
 * continuation). Owns termination signaling for LLM-generated responses — the only place that can
 * act on LLM output to set {@code finishReason} appropriately.
 *
 * <h3>Guarantees</h3>
 *
 * <ul>
 *   <li><b>Partial vs. final response handling:</b> Partial responses (streaming deltas) receive
 *       {@code turnComplete=false}. Non-partial responses (final assembly) receive {@code
 *       turnComplete=true}.
 *   <li><b>Termination on final answers:</b>
 *       <ul>
 *         <li>If no continuation is requested: response has {@code finishReason=STOP}.
 *         <li>If continuation is requested: the retry continues unless the configured turn limit is
 *             reached.
 *       </ul>
 *   <li><b>Tool call handling:</b> Tool-call responses (non-final-answer) never have {@code
 *       finishReason} set unless the turn limit is reached. The loop continues regardless of
 *       continuation requests for tool calls.
 *   <li><b>Turn limit enforcement:</b> Any non-partial response that keeps the loop alive
 *       (tool-call steps or continuation-driven retries) consumes a turn. When the limit is
 *       reached, the response receives {@code finishReason=STOP} to terminate the loop.
 * </ul>
 *
 * <h3>Expectations from upstream</h3>
 *
 * <ul>
 *   <li>Partial responses must be properly marked: {@code response.partial().orElse(false)} must
 *       reliably indicate whether a response is a streaming delta or final assembly.
 * </ul>
 *
 * <h3>State Management</h3>
 *
 * <p>Turn consumption is tracked in {@link com.agentengine.engine.utils.RunState} (scoped to the
 * current invocation). Any non-partial response that does not already terminate the loop consumes a
 * turn. The limit applies per run — it resets with each new user invocation.
 */

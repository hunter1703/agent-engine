...
import com.agentengine.runtime.actor.SessionRunner;
...
public class RunnerFactory {
    ...
    public RunnerFactory(AgentService agentService, AgentProvider agentProvider, ContextManagerProvider contextManagerProvider, GuardrailPolicyFactory guardrailPolicyFactory, ProjectionBackedSessionService sessionService) {
    ...
    public SessionRunner buildRunner(final String agentId, final String sessionId, final ActorRef<SessionCommand> actor) {
        ...
        final App app = App.builder().plugins(buildPlugins(agent)).rootAgent(agent).name(agentId).resumabilityConfig(new ResumabilityConfig(true)).build();
        final Runner runner = Runner.builder().app(app).artifactService(new InMemoryArtifactService()).sessionService(sessionService).memoryService(new InMemoryMemoryService()).build();

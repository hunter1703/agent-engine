...
public final class GuardrailPlugin extends BasePlugin {
    ...
    public Maybe<LlmResponse> beforeModelCallback(
        ...
        final String text =
                ContentUtils.extractLatestUserText(llmRequestBuilder.build().contents());
        if (StringUtils.isBlank(text)) {
            return Maybe.empty();
        }
        final GuardrailDecision decision =
                GuardrailUtils.evaluate(new GuardrailContext(text, invocationContext), guardrails, policy.errorMode());
    ...
    public Maybe<LlmResponse> afterModelCallback(final CallbackContext callbackContext, final LlmResponse llmResponse) {
        ...
        final GuardrailDecision decision = GuardrailUtils.evaluate(
                new GuardrailContext(content.text(), invocationContext), guardrails, policy.errorMode());
    ...
    private static boolean requiresRegeneration(final GuardrailDecision decision) {
        ...
        if (Boolean.TRUE.equals(
                CollectionUtils.getBooleanValueFromMap(details, GuardrailConstants.DetailKey.RETRY_REQUIRED))) {
            return true;
        }

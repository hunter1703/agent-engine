 ...
 *
 * <p>Subclasses call {@link #processImage(String, Function, ToolContext)} from their {@code execute}
 * method, passing the input artifact name and a function that receives the downloaded temp file and
 * returns the processed output temp file. This class handles everything else: artifact loading,
 * format detection, temp file lifecycle, and saving the result as an ADK artifact.
 */
public abstract class ImageEditingTool extends Tool {
    ...
    /**
     * Loads the artifact at {@code artifactName}, applies {@code adjustment}, and saves the result
     * as a new ADK artifact so the model can view it via {@code load_artifacts}.
     *
     ...
     */
    protected Map<String, Object> processImage(
            final String artifactName, final Function<File, File> adjustment, final ToolContext toolContext) {
        try {
            final Part input = toolContext.loadArtifact(artifactName, Optional.empty()).blockingGet();
            if (input == null) {
                return Map.of("error", "Artifact not found: " + artifactName);
            }
            final byte[] inputBytes = input.inlineData()
            ...
            final String format = mimeType.contains("/") ? mimeType.substring(mimeType.lastIndexOf('/') + 1) : "png";
            ...
            try {
                ...
                try {
                    ...
                    toolContext.saveArtifact(outputName, Part.fromBytes(outputBytes, "image/png")).blockingAwait();
                    return Map.of("artifactName", outputName);
                } finally {

 ...
 * result, or {@link SendMessageTool} to assign follow-up tasks while preserving
...
public final class SpawnAgentTool extends AbstractAgentTool {
  ...
  public Map<String, Object> execute(
    ...
    final SessionReply.SpawnResult result = actorRef(toolContext).<SessionReply.SpawnResult>ask(
        replyTo -> new SessionCommand.ExternalCommand.SpawnChild(childAgentId, message, replyTo), ActorUtils.DEFAULT_ASK_TIMEOUT)
        .toCompletableFuture().join();

    return switch (result) {
      case SessionReply.SpawnResult.ChildSpawned(var handle) ->
        Map.of("child_session_id", handle.childSessionId(), "child_run_id", handle.childRunId());
      case SessionReply.SpawnResult.Rejected(var reason) -> Map.of("error", "Failed to spawn agent: " + reason);
    };
  }
}

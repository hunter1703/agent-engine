 ...
 * result, or {@link SendMessageTool} to send follow-up messages while
 * preserving its context. Both tools require the child_session_id returned by
 * this tool. This tool is injected per-run; it is not a CDI singleton.
 */
public final class SpawnAgentTool extends AbstractAgentTool {
  ...
  public Map<String, Object> execute(
    ...
    final StartChildResult startChildResult = actorRef(toolContext)
        .<StartChildResult>ask(replyTo -> new ExternalCommand.StartChildCommand(childAgentId, message, replyTo),

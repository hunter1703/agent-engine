...
/**
 * Caps each input chunk to a maximum estimated token count (approximated as chars / 3).
 * Oversized chunks are split at sentence boundaries.
 */
public final class MaxTokenCapStage extends ChunkingStage {
    ...
    public List<KnowledgeChunk> apply(final List<KnowledgeChunk> chunks) {
        ...
        return ChunkUtils.splitChunks(chunks, text -> {
    ...
    private static List<String> splitBySentence(final String text, final int maxChars) {
        final DocumentBySentenceSplitter splitter = new DocumentBySentenceSplitter(Integer.MAX_VALUE, 0);
        final List<String> sentences = splitter.split(Document.from(text)).stream()

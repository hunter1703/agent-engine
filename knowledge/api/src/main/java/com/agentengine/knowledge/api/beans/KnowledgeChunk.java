...
/**
 * A single indexed chunk stored in the vector store.
 * The {@code id} is the point ID ({@code knowledgeId + "-" + chunkIndex}).
 *
 * <p>Source and sourceType are intentionally omitted — they are available on the parent
 * {@link Knowledge} entity and can be fetched via {@code knowledgeId} when needed.
 *
 * <p>Currently a single vector field is used ({@link #FIELD_TEXT_VECTOR}), populated by embedding
 * the chunk's text. Additional fields (e.g. title, summary) can be added later by calling
 * {@link #setVector(String, float[])} with the appropriate physical field name.
 */

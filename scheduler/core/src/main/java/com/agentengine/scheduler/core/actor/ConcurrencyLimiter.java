 ...
 *
 * <p>A permit reserves every one of its keys or none of them, so an item held up by one exhausted key
 * does not consume capacity in the others while it waits. Reservations carry the time they were
 ...
 *
 * <p>Thread-safe: the node-scoped instance is shared by every runner entity on the pod, and
 * {@link #tryAcquire} is all-or-nothing across keys, which no concurrent map can make atomic on its
 * own. Contention is negligible — one acquire and one release per dispatch.
 */
public final class ConcurrencyLimiter {
    ...
    public synchronized boolean tryAcquire(final List<String> keys, final String permitId, final long now) {
    ...
    public synchronized int evictExpired(final long now) {
        int evicted = 0;
        final Iterator<Entry<String, Map<String, Long>>> keys =
                keyVsPermits.entrySet().iterator();
        while (keys.hasNext()) {

...
public final class DefaultResponseMapper {
  ...
  private static final Set<Integer> DEFAULT_SUCCESS_STATUS_CODES =
      java.util.stream.IntStream.range(200, 300)
          .boxed()
  ...
  public DefaultResponseMapper(
      final ResponseExtractor responseExtractor, final ErrorClassifier errorClassifier) {
  ...
  public ConnectorExecutionResult map(
    ...
    final Object data =
        extractor.extract(responseData.body(), definition.responseMapping().dataJsonPath());
    final Object metadata =
        extractor.extract(responseData.body(), definition.responseMapping().metadataJsonPath());

...
public final class AuthStrategyRegistry {
  ...
  public void apply(final AuthRequestContext requestContext) {
    ...
    final AuthConfig effectiveConfig = configured == null
        ...
        : configured;
    final AuthType effectiveType = effectiveConfig.typeEnum() == AuthType.UNKNOWN ? AuthType.NONE : effectiveConfig.typeEnum();
    ...
    strategy.apply(new AuthRequestContext(effectiveConfig, requestContext.requestContext(), requestContext.templateResolver(),
        requestContext.strictUnresolvedVariables(), requestContext.headers(), requestContext.queryParams()));
  }

...
public final class BasicAuthStrategy implements AuthStrategy {
  ...
  public void apply(final AuthRequestContext requestContext) {
    ...
    final String username = AuthTemplateUtils.resolveString(authConfig.username(), authConfig.usernameTemplate(),
        requestContext.requestContext(), requestContext.templateResolver(), requestContext.strictUnresolvedVariables(), "username");
    final String password = AuthTemplateUtils.resolveString(authConfig.password(), authConfig.passwordTemplate(),
        requestContext.requestContext(), requestContext.templateResolver(), requestContext.strictUnresolvedVariables(), "password");

...
public final class OffsetPaginationStrategy implements PaginationStrategy {
  ...
  public PaginationDirective buildRequest(final PaginationConfig config, final PaginationState state) {
    return new PaginationDirective(
        Map.of(config.offsetParam(), String.valueOf(state.offset()), config.limitParam(), String.valueOf(config.limit())), null);
  }
  ...
  public PaginationState updateState(final PaginationConfig config, final PaginationState currentState, final String responseBody,
    ...
    final boolean doneByMaxPages = PaginationStrategySupport.reachedMaxPages(nextIteration, config.maxPages());
    return new PaginationState(nextIteration, currentState.pageNumber(), currentState.offset() + config.limit(), currentState.cursor(),
        currentState.nextPageUrl(), doneByData || doneByMaxPages);
  }
}

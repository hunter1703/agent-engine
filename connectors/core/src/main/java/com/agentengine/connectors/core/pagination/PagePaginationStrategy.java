...
public final class PagePaginationStrategy implements PaginationStrategy {
  ...
  public PaginationDirective buildRequest(final PaginationConfig config, final PaginationState state) {
    return new PaginationDirective(
        Map.of(config.pageParam(), String.valueOf(state.pageNumber()), config.pageSizeParam(), String.valueOf(config.pageSize())), null);
  }
  ...
  public PaginationState updateState(final PaginationConfig config, final PaginationState currentState, final String responseBody,
    ...
    return new PaginationState(nextIteration, currentState.pageNumber() + 1, currentState.offset(), currentState.cursor(),

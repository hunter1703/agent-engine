...
public final class DefaultConnectorExecutor implements ConnectorExecutor {
  ...
  @Inject
  public DefaultConnectorExecutor(final ConnectorConfigValidator connectorConfigValidator, final RequestMaterializer requestMaterializer,
      final HttpTransport httpTransport, final PaginationStrategyRegistry paginationStrategyRegistry,
  ...
  @Override
  public ConnectorExecutionResult executeOnce(final ConnectorDefinition definition, final RequestContext context) {
    validator.validateOrThrow(definition);
    return executeWithRetry(definition, context, PaginationDirective.empty());
  }

  @Override
  public PaginatedExecutionResult executeAllPages(final ConnectorDefinition definition, final RequestContext context) {
    validator.validateOrThrow(definition);
    ...
    if (paginationType == PaginationType.NONE) {
      ...
      return new PaginatedExecutionResult(List.of(singleResult), asItems(singleResult.mappedData()), false, 1);
    }

    final PaginationStrategy paginationStrategy = paginationRegistry.resolve(definition.pagination().typeEnum());
    ...
    while (!paginationState.done() && guard < definition.pagination().maxPages()) {
      final PaginationDirective directive = paginationStrategy.buildRequest(definition.pagination(), paginationState);
      ...
      paginationState = paginationStrategy.updateState(definition.pagination(), paginationState, pageResult.rawResponseBody(),
  ...
  private ConnectorExecutionResult executeWithRetry(final ConnectorDefinition definition, final RequestContext context,
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      final HttpRequestData requestData = materializer.materialize(definition, context, paginationDirective);
      try {
        final HttpResponseData responseData = transport.execute(requestData);
        final ConnectorExecutionResult mappedResult = responseMapper.map(definition, responseData, requestData.url(),
  ...
  private static boolean shouldRetry(final ConnectorDefinition definition, final ConnectorExecutionResult result, final int attempt,
  ...
  private long nextDelay(final ConnectorDefinition definition, final long currentDelayMs) {
    ...
    if (definition.retryPolicy().backoffTypeEnum() == RetryBackoffType.EXPONENTIAL) {
      final long nextDelay = Math.round(currentDelayMs * definition.retryPolicy().backoffMultiplier());

...
class DefaultConnectorServiceTest {
  ...
  void injectsAuthMaterialsIntoContextAuthAndInputAuth() {
    ...
    final DefaultConnectorService service = new DefaultConnectorService(connectorId -> Optional.of(connectorDefinition()), executor,
    ...
    assertThat(executor.capturedContext.input()).containsEntry("query", "hello").containsEntry("auth", Map.of("token", "secret-token"));
  }
  ...
  void preservesExistingInputAuthAndOverlaysRepositoryAuth() {
    ...
    assertThat(executor.capturedContext.auth()).containsEntry("token", "repo-token").containsEntry("tenant", "alpha");

    final Map<String, Object> inputAuth = (Map<String, Object>) executor.capturedContext.input().get("auth");
    assertThat(inputAuth).containsEntry("token", "repo-token").containsEntry("tenant", "alpha").containsEntry("region", "us");
  }

  private static ConnectorDefinition connectorDefinition() {
    return new ConnectorDefinition("test", "test-app",
        new EndpointConfig(HttpMethod.GET, "https://example.com", null, "/", null, null, null, null, true, true), Map.of(), Map.of(), null,
  ...
  private static final class CapturingExecutor implements ConnectorExecutor {
    ...
    public ConnectorExecutionResult executeOnce(final ConnectorDefinition definition, final RequestContext context) {
      ...
      return new ConnectorExecutionResult(200, true, "https://example.com", "GET", Map.of(), Map.of(), null, null, null, null, false);
    }

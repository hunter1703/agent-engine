...
public class HttpConnectorExecutor implements ConnectorExecutor<Map<String, Object>, Map<String, Object>> {
    ...
    public ConnectorResult<Map<String, Object>> execute(Map<String, Object> input) {
        ...
        final HttpUrl url = buildUrl(evaluated.getBaseUrl(), evaluated.getPath(), evaluated.getQueryParams());
        final String method = evaluated.getMethod();

        Map<String, String> headers = evaluated.getHeaders();
        final RequestBody requestBody = createRequestBody(method, headers.get("Content-Type"), evaluated.getBody());

        final Request.Builder requestBuilder = new Request.Builder().url(url).method(method, requestBody);
    ...
    private static List<Map<String, Object>> buildResult(final String mimeType, final byte[] responseBody) {
        if (mimeType == null) {

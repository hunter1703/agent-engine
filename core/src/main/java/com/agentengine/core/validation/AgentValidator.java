...
public class AgentValidator implements Validator<BaseAgentConfig> {
  ...
  private void validateOrchestratorSubAgentsExist(final OrchestratorAgentConfig config, final ValidationCollector errors) {
    if (CollectionUtils.isEmpty(config.getSubAgentIds())) {
      return;
    }
    final List<String> allSubAgentIds = config.getSubAgentIds().stream().filter(StringUtils::isNotBlank).toList();
    final Map<String, BaseAgentConfig> subAgents = agentRepository.get().findByIds(allSubAgentIds);
    final List<String> missing = allSubAgentIds.stream().filter(id -> !subAgents.containsKey(id)).toList();
    if (!missing.isEmpty()) {

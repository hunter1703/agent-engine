...
public class AgentExecutionServiceImpl implements AgentExecutionService {
  ...
  public Publisher<BaseEvent> run(final String agentId, final String text) {
    ...
    return Flowable.fromCompletionStage(sessionEventChannel.subscribe(sessionId))
        .flatMap(subscription -> Flowable.just(runtimeService.startSession(agentId, sessionId, text)).flatMap(startResult -> {
  ...
  public Publisher<BaseEvent> resumeSession(final String sessionId, final String confirmationId, final Boolean confirmed,
    ...
    final ResumeResult resumeStage = runtimeService.resumeSession(rootAgentId, rootSessionId, confirmationId,
        confirmed != null && confirmed, answer);

    return Flowable.fromCompletionStage(sessionEventChannel.subscribe(rootSessionId))
        .flatMap(subscription -> Flowable.just(resumeStage).flatMap(result -> {
  ...
  private static Publisher<BaseEvent> mapEvents(final String agentId, final String sessionId,
    ...
    return Flowable.fromPublisher(subscription.publisher()).concatMap(event -> mapper.map(event.payload()))
        .takeUntil(mapper::isTerminalEvent).doFinally(() -> cancelQuietly(subscription, null));
  }

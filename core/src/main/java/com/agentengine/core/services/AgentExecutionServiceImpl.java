...
public class AgentExecutionServiceImpl implements AgentExecutionService {
    ...
    @Inject
    public AgentExecutionServiceImpl(final SessionService sessionService,
    ...
    public Publisher<BaseEvent> run(final String agentId, final String text) {
        ...
        runtimeService.startSession(agentId, sessionId, text).whenComplete((receipt, ex) -> {
            if (ex != null) {
                // System failure (actor timeout, cluster down) — the actor cannot clean up; we must.
                LOG.error("Session {}:{} failed to start: {}", agentId, sessionId, ex.getMessage());
                sessionEventChannel.complete(sessionId);
            } else if (receipt instanceof SessionActor.RunReceipt.Rejected rejected) {
                // Rejection is handled by the actor, which calls eventChannel.complete() itself.
    ...
    @Override
    public Publisher<BaseEvent> resumeSession(final String sessionId, final String confirmationId,
        ...
        // Always resume from the root: the cascade unwinds top-down through the awaited hierarchy.
        ...
        return Flowable.fromCompletionStage(
                ...
                .concatMap(receipt -> switch (receipt) {
                    case SessionActor.RunReceipt.Accepted, SessionActor.RunReceipt.Queued _ -> Flowable.fromPublisher(channelEvents);
    ...
    private Publisher<BaseEvent> subscribe(final String agentId, final String sessionId) {
        ...
        return Flowable.fromPublisher(sessionEventChannel.events(sessionId))

...
public final class LastNContextBuilder extends BaseContextBuilder {
    ...
    public LastNContextBuilder(final SessionStore sessionStore, final String systemMessage, final String protocolMessage, final List<AgentTool> tools, final int keepLast) {
    ...
    public List<Message> buildPrompt(final String sessionId) {
        ...
        final List<Message> recent = messages.size() <= keepLast ? messages : messages.subList(messages.size() - keepLast, messages.size());
        final Map<String, List<ToolExecution>> messageIdVsToolExecutions = sessionStore.getToolExecutions(sessionId, recent.stream().map(Message::getId).collect(Collectors.toList()));

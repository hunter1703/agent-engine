...
public final class EngineUtils {
  ...
  private static final Pattern FINAL_BLOCK =
      Pattern.compile("FINAL:\\s*(.*)$", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
  private static final Pattern TOOL_REQUEST_BLOCK =
      Pattern.compile(
          "TOOL_REQUEST:\\s*(.*?)(?=\\n\\s*TOOL_REQUEST:|\\n\\s*FINAL:|$)",
  ...
  public static Message sanitizeMessage(
    ...
    final String cleaned =
        stripThoughtBlock(content, thoughtsEnabled, thoughtsStartTag, thoughtsEndTag);
  ...
  public static String getRepairMessageIfInvalid(final Message message) {
    ...
    final boolean finalAnswerAndToolCallsPresent =
        StringUtils.isNotBlank(content) && CollectionUtils.isNotEmpty(toolRequests);
    final boolean emptyResponse =
        StringUtils.isBlank(content)
            && StringUtils.isBlank(thoughts)
  ...
  private static List<ToolCall> parseToolCallsFromJsonMap(final Map<String, Object> payload) {
    ...
    for (Object item : list) {
      ...
      calls.add(
          new ToolCall(idValue == null ? null : idValue.toString(), nameValue.toString(), args));
    }
    return calls;
  }

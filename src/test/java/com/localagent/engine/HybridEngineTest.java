...
class HybridEngineTest {
    ...
    void invokeRunsToolPlanAndReturnsFinalAnswer() {
        ...
        LLMModel reasoningModel = new QueueModel(
        ...
        LLMModel toolAssistantModel =
                new QueueModel(ResponseFormatType.JSON, List.of(new Message(Role.ASSISTANT, toolAssistantPayload, null, null, null)));
        ...
        HybridEngine engine =
                new HybridEngine(reasoningModel, toolAssistantModel, tools, reasoningContext, toolContext, sessionStore, 2);
        ...
        Message toolRequestMessage = reasoningMessages.stream()
                .filter(message -> message.getToolRequests() != null && !message.getToolRequests().isEmpty())
                .findFirst()
                .orElseThrow();
    ...
    void invokeTriggersToolRepairAndHandlesToolFailure() {
        ...
        LLMModel toolAssistantModel = new QueueModel(
    ...
    void invokeReturnsNullWhenInvocationLimitReached() {
        ...
        BaseContextBuilder toolContext = new BaseContextBuilder(sessionStore, "system", "protocol", tools);
        HybridEngine engine =
                new HybridEngine(reasoningModel, toolAssistantModel, tools, reasoningContext, toolContext, sessionStore, 1);
    ...
    private static final class CapturingListener implements AgentListener {
        private final List<List<com.localagent.engine.message.ToolCall>> toolPlans = new ArrayList<>();
        private final List<ToolExecution> toolExecutions = new ArrayList<>();
        private int toolRepairs = 0;

        @Override
        public void onToolPlan(final String sessionId, final List<com.localagent.engine.message.ToolCall> toolCalls) {

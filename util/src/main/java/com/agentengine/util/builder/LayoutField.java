...
public record LayoutField(
  ...
  public LayoutField withCurrentAccess(final String accessLevel) {
    return new LayoutField(
        label, widget, step, section, order, access, collection,
        multiline, rows, numberType, advanced, options, rules, lookup,
        presets, dynamicSchema, sensitive, accessLevel);
  }
  ...
  public LayoutField withPresets(final List<LayoutPreset> fieldPresets) {
    return new LayoutField(
        ...
        dynamicSchema, sensitive, currentAccess);
  }
}

...
public @interface UiDynamicSchema {
   ...
   * Example: {@code "$item.toolName"} resolves to the {@code toolName} of the current list item.
   */
  String assetIdExpr() default "";

  /**
   * JSONPath expression to resolve the context (parent entity) ID.
   * Example: {@code "$.id"} resolves to the ID of the root entity being edited.
   */

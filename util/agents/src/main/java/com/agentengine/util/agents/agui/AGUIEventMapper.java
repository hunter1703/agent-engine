...
public final class AGUIEventMapper implements EventMapper<SessionEvent, BaseEvent> {
  ...
  public boolean isTerminalEvent(final BaseEvent event) {
    ...
    final String threadId = CollectionUtils.getStringValueFromMap((Map<String, Object>) event.getRawEvent(), "threadId");
    return (event instanceof RunFinishedEvent || event instanceof RunErrorEvent) && Objects.equals(state.sessionId(), threadId);
  }
  ...
  private Flowable<BaseEvent> mapPart(final Part part, final boolean partial, final boolean internal) {
    ...
    if (call != null) {
      flowable = flowable.concatWith(textMapper.closeReasoningIfNeeded()).concatWith(toolCallMapper.mapToolCall(call));
    }

    final FunctionResponse response = part.functionResponse().orElse(null);
    if (response != null) {
      flowable = flowable.concatWith(textMapper.closeReasoningIfNeeded()).concatWith(toolCallMapper.mapToolResponse(response));
    }
    return flowable;
  }

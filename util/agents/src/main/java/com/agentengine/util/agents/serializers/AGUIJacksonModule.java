...
public final class AGUIJacksonModule extends SimpleModule {

  public AGUIJacksonModule() {
    super("AGUIJacksonModule");
  ...
  /**
   * Reads {@link Role} back from its lowercase wire value — the inverse of {@link RoleSerializer}.
   * Without this, Jackson's default enum deserializer only accepts the constant's own Java name
   * ({@code "ASSISTANT"}), not the lowercase value {@link RoleSerializer} actually writes.
   */
   ...
   * deserializer to do the actual binding, rather than duplicating its field list here (which would
   * drift out of sync with the library's own shape over time) or letting its {@code requireNonNull}
   * constructor checks reject an otherwise-valid partial payload. The AG-UI protocol allows a
   * client to omit {@code threadId}/{@code runId}/message {@code id} when starting a new run — the
   * server is expected to allocate them — but {@code RunAgentInput} (from {@code
   * com.ag-ui.community:java-core}, not our code) requires them non-null at construction. {@code
   * RuntimeServiceImpl} already treats a blank {@code threadId} as "create a new session", so
   * defaulting to {@code ""} here matches an existing convention rather than inventing one.
   *

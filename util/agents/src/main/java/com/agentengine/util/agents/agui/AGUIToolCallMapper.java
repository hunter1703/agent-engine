...
public final class AGUIToolCallMapper {
    ...
    public Flowable<Event> mapToolCall(final FunctionCall call) {
        ...
        final ToolCallStartEvent start =
                new ToolCallStartEvent(callId, toolName, state.currentTextMessageId(), state.timestamp(), null);
    ...
    public Flowable<Event> mapToolResponse(final FunctionResponse response) {
        ...
        final ToolCallResultEvent result = new ToolCallResultEvent(
                state.nextToolResultMessageId(), callId, contentResult, Role.TOOL, state.timestamp(), null);
    ...
    public Flowable<Event> mapResumedResponse(final FunctionResponse response) {
        ...
        final FunctionCall interruptRequestedCall = Objects.requireNonNull(
    ...
    /**
     * Resolves the tool call opened for {@code await_agent} in {@link #mapToolCall} with the
     * child run's actual result, instead of letting it dangle forever: the outer confirmation
     * machinery that carries this result is otherwise entirely suppressed from the client (see
     * above), so without this the client never learns the awaited tool call finished.
     */
    private Flowable<Event> mapAwaitAgentResult(
        ...
        final ToolCallResultEvent result = new ToolCallResultEvent(
                state.nextToolResultMessageId(), callId, contentResult, Role.TOOL, state.timestamp(), null);
        LOG.debug("Generated output event - eventType=ToolCallResultEvent (await_agent), callId={}", callId);
        return Flowable.just(result);
    }
}

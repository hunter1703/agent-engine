...
public final class AGUITextMapper {
  ...
  public Flowable<BaseEvent> mapText(final String text, final boolean partial) {
    ...
    return closeReasoningIfNeeded().concatWith(startTextMessageIfNeeded()).concatWith(mapTextMessageContent(text, partial))
  ...
  public Flowable<BaseEvent> mapUserMessage(final SessionEvent event) {
    final String text = event.getContent() != null
        ? event.getContent().parts()
            .flatMap(parts -> parts.stream().map(part -> part.text().orElse("")).filter(StringUtils::isNotBlank).findFirst()).orElse(null)
        : null;

 ...
 * (backed by {@code INFRA.InfraConfig} in MongoDB), mirroring the pattern used by {@code
 * MongoClientFactory} and {@code LocalStackCloudStorageService}.
 *
...
public class VectorDbClientFactory {
  ...
  public VectorDbClientFactory(final InfraConfigService infraConfigService) {
    this.client =
        new LazyLoader<>(
            () -> {
              final VectorDatabaseInfraConfig config =

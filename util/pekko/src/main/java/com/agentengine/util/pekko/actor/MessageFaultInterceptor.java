 ...
 * ChaosMailboxConfig} is currently registered for {@code entityId} in {@link ChaosMailboxRegistry}.
 * The registry is consulted fresh on every message, so activating or clearing a fault takes effect
 * immediately without an actor restart. Lifecycle signals are never intercepted — only {@code
 * aroundReceive} is overridden.
 *
 ...
 * forever. {@code inFlightRedeliveries} (identity-based, since a single actor processes one message
 * at a time — no synchronization needed) marks a message as "already delayed once" so its
...
public final class MessageFaultInterceptor<Command> extends BehaviorInterceptor<Command, Command> {
  ...
  private final Set<Command> inFlightRedeliveries =
      Collections.newSetFromMap(new IdentityHashMap<>());

  public MessageFaultInterceptor(
      final Class<Command> commandClass,
      final String entityId,
      final ChaosMailboxRegistry registry) {
    super(commandClass);

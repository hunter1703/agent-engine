...
/**
 * Base factory for cluster-sharded entities. Handles sharding registration and
 * entity ref acquisition. The {@code builder} function receives the
 * {@link EntityContext} and should return a {@link Behavior} — typically a
 * {@code Behaviors.setup()} wrapper so that the entity can receive an
 * {@code ActorContext} in its constructor.
 *
 * @param <Command>
 *          the entity's command type
 */
public abstract class ShardedEntityFactory<Command> implements ShardedEntity.ShardedEntityDefinition<Command, ShardingEnvelope<Command>> {
  ...
  protected ShardedEntityFactory(final ActorSystem<SpawnProtocol.Command> actorSystem, final EntityTypeKey<Command> entityTypeKey,
  ...
  private Entity<Command, ShardingEnvelope<Command>> buildEntity(final EntityTypeKey<Command> entityTypeKey,
      final Function<EntityContext<Command>, Behavior<Command>> builder, final Duration passivationDuration) {

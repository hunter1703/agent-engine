 ...
 *
 * <ul>
 *   <li>A fresh subscription ({@code lastSeenSequence == null}) receives no backlog: the
 *       broadcaster returns {@code replayAccepted=false} and an empty backlog. The actor
 *       initialises its checkpoint to the broadcaster's current {@code latestAvailableSequence} so
 *       that only live events published after subscription are delivered.
 *   <li>A re-subscription ({@code lastSeenSequence != null}) requests a catch-up replay from the
 *       broadcaster. If the gap exceeds the broadcaster's bounded retention window, the
 *       subscription fails with an unrecoverable error.
 * </ul>
 *
...
final class SubscriberActor extends AbstractBehavior<SubscriberCommand> {
        ...
        .pipeToSelf(
            broadcasterEntity.ask(
                (Function<ActorRef<SubscribeAck>, BroadcasterCommand>)
                    replyTo ->
                        new BroadcasterCommand.SubscribeCommand(
                            subscriptionId, lastSeenSequence, getContext().getSelf(), replyTo),
                COMMAND_TIMEOUT),
  ...
  private Behavior<SubscriberCommand> onBroadcasterTerminated(
      final BroadcasterTerminatedCommand command) {
    LOG.warn(
        "Broadcaster terminated for id={}, resubscribing in {}ms",
        subscriptionId,
        RETRY_DELAY.toMillis());
    broadcaster = null;
    getContext().scheduleOnce(RETRY_DELAY, getContext().getSelf(), new ResubscribeCommand());

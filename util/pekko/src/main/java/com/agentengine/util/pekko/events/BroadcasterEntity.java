...
public final class BroadcasterEntity extends ShardedEntity<BroadcasterCommand, BroadcasterFact, BroadcasterState> {
  ...
  public BroadcasterEntity(final ActorContext<BroadcasterCommand> context, final String typeKeyName, final String entityId) {
  ...
  public CommandHandler<BroadcasterCommand, BroadcasterFact, BroadcasterState> commandHandler() {
    return newCommandHandlerBuilder().forAnyState().onCommand(BroadcasterCommand.SubscribeCommand.class, this::subscribe)
        .onCommand(BroadcasterCommand.PublishCommand.class, this::publish)
        .onCommand(BroadcasterCommand.UnsubscribeCommand.class, this::unsubscribe).build();
  }

  @Override
  public EventHandler<BroadcasterState, BroadcasterFact> eventHandler() {
    return newEventHandlerBuilder().forAnyState()
        .onEvent(BroadcasterFact.PublishedFact.class, (state, fact) -> state.withPublished(fact.event(), MAX_RETAINED_EVENTS)).build();
  }

  private Effect<BroadcasterFact, BroadcasterState> subscribe(final BroadcasterState state,
      final BroadcasterCommand.SubscribeCommand command) {
    ...
    return Effect().none().thenRun(ignored -> {
      ...
      command.replyTo().tell(new SubscribeAck(context.getSelf(), replayAccepted, state.oldestRetainedSequence(),
          state.latestPublishedSequence(), state.eventsAfter(command.lastSeenSequence())));
    });
  }

  private Effect<BroadcasterFact, BroadcasterState> publish(final BroadcasterState state,
      final BroadcasterCommand.PublishCommand<?> command) {

    final SequencedEvent<?> event = new SequencedEvent<>(state.latestPublishedSequence() + 1, command.payload());
    return Effect().persist(new BroadcasterFact.PublishedFact(event))
        .thenRun(ignored -> subscribers.forEach((_, subscriber) -> subscriber.tell(new SubscriberCommand.DeliverCommand(event))))
        .thenReply(command.replyTo(), ignored -> new PublishAck(event.sequence()));
  }

  private Effect<BroadcasterFact, BroadcasterState> unsubscribe(final BroadcasterState state,
      final BroadcasterCommand.UnsubscribeCommand command) {

...
public class LocalStackCloudStorageService implements CloudStorageService {
              ...
              final StaticCredentialsProvider credentials =
                  StaticCredentialsProvider.create(
                      AwsBasicCredentials.create(
                          cloudStorageInfraConfig.getAccessKeyId(),
                          cloudStorageInfraConfig.getSecretAccessKey()));
  ...
  public void delete(final String key) {
    s3.get()
        .deleteObject(DeleteObjectRequest.builder().bucket(defaultBucket.get()).key(key).build());
  }
    ...
    return presigner
        ...
        .presignGetObject(
            GetObjectPresignRequest.builder()
                ...
                .getObjectRequest(request -> request.bucket(defaultBucket.get()).key(key))
                .build())
        .url()
        .toString();
  }
  ...
  public List<String> list(final String keyPrefix) {
    return s3
        ...
        .listObjectsV2Paginator(
            ListObjectsV2Request.builder().bucket(defaultBucket.get()).prefix(keyPrefix).build())
        .stream()

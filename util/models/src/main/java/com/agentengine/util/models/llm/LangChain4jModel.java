...
public final class LangChain4jModel extends BaseLlm {
  ...
  private static void applyConfig(
    ...
    config
        .presencePenalty()
        .ifPresent(presencePenalty -> builder.presencePenalty(presencePenalty.doubleValue()));
    config
        .toolConfig()
        .ifPresent(
            toolConfiguration -> applyToolConfig(builder, toolConfiguration, toolSpecifications));
    config
        .responseJsonSchema()
        .ifPresent(schema -> builder.responseFormat(toResponseFormat(schema)));
  }

  @SuppressWarnings("unchecked")
  private static ResponseFormat toResponseFormat(final Object responseJsonSchema) {
    final JsonSchema jsonSchema = toJsonSchema((Map<String, Object>) responseJsonSchema);
    return ResponseFormat.builder().type(ResponseFormatType.JSON).jsonSchema(jsonSchema).build();
  }

  private static JsonSchema toJsonSchema(final Map<String, Object> jsonSchemaMap) {
    return JsonSchema.builder().rootElement(buildJsonSchemaElement(jsonSchemaMap)).build();
  }

  private static JsonSchemaElement buildJsonSchemaElement(final Map<String, Object> jsonSchema) {
    if (jsonSchema == null) {
  ...
  private static List<ChatMessage> toUserOrToolResultMessages(final Content content) {
    ...
    // it is a UserMessage.
    final List<ChatMessage> toolResults =
        parts.stream()
  ...
  private static dev.langchain4j.data.message.Content toMediaContent(final Blob blob) {
    final String mimeType =
        blob.mimeType().orElseThrow(() -> new IllegalArgumentException("Mime type required"));
    final byte[] bytes =
        blob.data().orElseThrow(() -> new IllegalArgumentException("Data required"));
            ...
            part -> {
              if (part.text().isPresent()) {
                ...
                throw new IllegalStateException(
                    "Either text or functionCall expected, but was: " + part);
              }
            });
        ...
        .map(
            tool -> {
              final FunctionDeclaration decl =
                  tool.declaration()
                      .orElseThrow(
                          () -> new IllegalStateException("Tool lacking declaration: " + tool));
              final Schema schema =
                  decl.parameters()
                      .orElseThrow(
                          () -> new IllegalStateException("Tool lacking parameters: " + tool));

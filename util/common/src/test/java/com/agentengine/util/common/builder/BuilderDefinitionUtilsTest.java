...
class BuilderDefinitionUtilsTest {
  ...
  void shouldOnlyExposeAnnotatedFieldsAndMetadata() {
    ...
    assertThat(definition.layout().fields()).containsKeys("/id", "/systemPrompt", "/mode", "/toolIds", "/configs", "/retries", "/nested",
        "/requiredViaNotNull", "/requiredViaAccess");
    ...
    assertThat(definition.layout().fields().get("/mode")).extracting(LayoutField::widget, LayoutField::options).containsExactly("SELECT",
        List.of("FAST", "SAFE"));

    assertThat(definition.layout().fields().get("/toolIds")).extracting(LayoutField::widget, LayoutField::collection)
        .containsExactly("LOOKUP", Boolean.TRUE);
    assertThat(definition.layout().fields().get("/toolIds").lookup()).extracting(LayoutLookup::assetType, LayoutLookup::multiSelect)
    ...
    assertThat(definition.layout().fields().get("/configs").dynamicSchema())
        .extracting(LayoutDynamicSchema::url, LayoutDynamicSchema::method).containsExactly("/schemas", "POST");
    assertThat(definition.layout().fields().get("/configs").dynamicSchema().body()).containsEntry("assetType", "tool_configs")
        .containsEntry("assetId", "$item.toolName").containsEntry("contextId", "$.id");
  }
  ...
  void shouldMarkFieldsRequiredFromValidationAndAccessRules() {
    ...
    assertThat(requiredFields(definition.schema())).contains("systemPrompt", "requiredViaNotNull", "requiredViaAccess")
  ...
  void shouldBuildRulesFromUiRuleAndUiExpression() {
    ...
    final LayoutFieldRule visible = retries.rules().stream().filter(rule -> "VISIBLE".equals(rule.effect())).findFirst().orElseThrow();
    final LayoutFieldRule required = retries.rules().stream().filter(rule -> "REQUIRED".equals(rule.effect())).findFirst().orElseThrow();
  ...
  void shouldSanitizeReadOnlyAndHiddenFieldsPerMode() {
    ...
    final SanitizeFixture sanitized = BuilderDefinitionUtils.sanitize(definition, BuilderMode.EDIT, payload);
  ...
  private static final class SampleConfig {
    ...
    @UiField
    @UiRule(effect = UiRuleEffect.VISIBLE, field = "mode", operator = UiConditionOperator.IN, values = {"FAST", "SAFE"})
    ...
    @UiField
    @NotNull
    private Integer requiredViaNotNull;

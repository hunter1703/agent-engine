...
class BuilderDefinitionUtilsTest {
    ...
    void shouldBuildRulesFromUiRuleAndUiExpression() {
        ...
        final LayoutFieldRule visible = retries.rules().stream()
                .filter(rule -> "VISIBLE".equals(rule.effect()))
                .findFirst()
                .orElseThrow();
        final LayoutFieldRule required = retries.rules().stream()
    ...
    void shouldAddAutoVisibilityRulesForPolymorphicSubtypeFields() {
        ...
        final List<?> whiskersArgs =
                (List<?>) whiskers.rules().getFirst().expr().get("===");

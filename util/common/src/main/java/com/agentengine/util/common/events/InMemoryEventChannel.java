...
public class InMemoryEventChannel<Scope, Event> implements EventChannel<Scope, Event> {
  ...
  private static final class ScopeState<E> {
    ...
    private final FlowableProcessor<Runnable> commands = PublishProcessor.<Runnable>create().toSerialized();
    ...
    private long doPublish(final E event) {
      ...
      final Iterator<Map.Entry<String, SubscriptionState<E>>> iterator = subscriptions.entrySet().iterator();

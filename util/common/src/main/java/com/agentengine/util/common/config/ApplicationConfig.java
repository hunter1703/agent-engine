...
public final class ApplicationConfig {
  ...
  private static void loadPropertiesFromClasspath(final Properties loadedProperties) {
    try (InputStream inputStream =
      ...
      if (inputStream == null) {
        LOG.warn(
            "Application config not found in classpath at '{}' - skipping", CLASSPATH_CONFIG_PATH);
        return;
      }
      ...
      LOG.warn(
          "Failed loading classpath application config '{}'", CLASSPATH_CONFIG_PATH, exception);
    }
  }

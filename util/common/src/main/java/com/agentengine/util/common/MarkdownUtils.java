 ...
 *   <li>{@link #fromHtml(String)} — converts HTML to Markdown via flexmark's {@link
...
public final class MarkdownUtils {
  ...
  private static List<Node> objectNodes(
      final JsonNode jsonNode, final int depth, final String label) {
  ...
  private static List<Node> arrayNodes(
      final JsonNode jsonNode, final int depth, final String label) {
    final List<Node> nodes = new ArrayList<>();
    if (label != null) {
      nodes.add(labelNode(label, depth));
  ...
  private static List<Node> scalarNodes(
      final JsonNode jsonNode, final int depth, final String label) {
  ...
  /**
   * Returns a {@link Heading} node for depth ≤ {@value MAX_HEADING_LEVEL}, or a bold {@link
  ...
  /**
   * Converts a camelCase or snake_case field name to a human-readable title. e.g. {@code
   * "subViolations"} → {@code "Sub Violations"}, {@code "error_code"} → {@code "Error Code"}.
   */

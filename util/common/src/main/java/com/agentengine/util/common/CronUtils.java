...
public final class CronUtils {
  ...
  public static Optional<Instant> nextTriggerTime(
      final String cronExpression, final Instant after) {
   ...
   */
  public static Instant applyJitter(
      final Instant exact, final Instant notBefore, final double jitterFraction) {
    if (jitterFraction <= 0) {
      return exact;
    }
    ...
    final long range =
        Math.max((long) ((exactMillis - floorMillis) * jitterFraction), MIN_JITTER_MILLIS);
